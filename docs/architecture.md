...
- **TAccessor**: Binds props/events of each unit (`{ instance, props, events }` — the component, its plugins) to that instance
  - `getProps(includeProtected?)` - Properties (`TProperty`)
  - `getEvents()` - Events (`IAccessorEvent`: `name`, `instance`, `source`)
- **TProperty**: a property together with its owner — declaration + instance. Читает, пишет и следит за свойством оно само, поэтому правило записи одно на сборку, связку и плагины снаружи
  - `value` - `get` декларации, а без него `instance[name]`; составное значение — снимком `valueOf()`
  - `assign(value)` - запись; protected и то же значение пропускаются
  - `reset()` - вернуть умолчание декларации; не объявлено — значение остаётся
  - `initialize(value)` - начальное значение: `undefined` и равное умолчанию ничего не задают
  - `watch(listener)` - слушать триггеры свойства на шине владельца; `source` - Event source (`instance.events` or the instance itself); `undefined` without `on`/`off`
...
- [property.class.ts](../packages/accessor/property.class.ts) - TProperty: чтение, запись и слежение за одним свойством
- [contract/types.ts](../packages/accessor/contract/types.ts) - TName, IContribution, declarations, IAccessorUnit, IAccessorEvent, INamingStrategy
...
- plugins of the mount (`IComponentPlugins`) — the one fork of the assembly, nobody else asks «whose bundle is it»:
  - `TOwnPlugins` — composition (`resolveComposition`: descriptor plugins, then app registrations `usePlugins`, `useTheme`), `bundle` built by it (`assembleBundle`; `bundle:create` and the plugins' `create` go out on a microtask), initial values of plugin props, plugins installed from outside (`TExternalPlugins`), destruction of the bundle
  - `TSharedPlugins` — the owner's bundle (`config.bundle`: a collection facade shares the component's bundle); composition is the descriptor plugins and is not resolved again, every other step is empty — the owner of the bundle did it
- `accessor` (`assembleAccessor`) — units of the instance and of the descriptor plugins
- initial values of props (`applyInitialProps`) — the one place they are applied, the same for all six adapters: an own instance got the core props through the constructor and they are not written again, an external `ctrl` and the plugins get them through `TProperty.initialize` — the setter (which emits the trigger) gets every prop that differs from the declared default
...
- [binding/component-binding.class.ts](../packages/setup/adapter/binding/component-binding.class.ts) - The binding `bindComponent` (Layer 5b): a facade that joins the surface with the accessor once and hands the pairs to three objects, one per direction
- [binding/state-store.class.ts](../packages/setup/adapter/binding/state-store.class.ts) - `TStateStore`, core → framework: `subscribe` / `getSnapshot`
- [binding/input-writer.class.ts](../packages/setup/adapter/binding/input-writer.class.ts) - `TInputWriter`, framework → core: `write` / `writeAll` / `writeChanged` and the memory of assigned inputs; `pluginProps` is an input sink (`IInputSink`) of its own, not a branch
- [accessor/event-relay.class.ts](../packages/accessor/event-relay.class.ts) - `TEventRelay`: event subscriptions without repeats by «source, name»; used by `bindEvents` and by `TExternalPlugins`
- [assemble/own-plugins.class.ts](../packages/setup/assemble/own-plugins.class.ts) / [assemble/shared-plugins.class.ts](../packages/setup/assemble/shared-plugins.class.ts) - Plugins of the mount (`IComponentPlugins`)
- [assemble/external-plugins.class.ts](../packages/setup/assemble/external-plugins.class.ts) - `TExternalPlugins`: `pluginProps` and `plugin:event` of plugins installed from outside
...
- через `valueOf()` — `TClasses.valueOf()` и `TCollectionStorageDriver.valueOf()`;
  `TProperty.value` аксессора вызывает его сам (`val?.valueOf?.() ?? val`);
...
(`pluginContractOf`). Набор монтирования (`TExternalPlugins` в `assemble/`) подписан на `use` и
...
- `useAdapter(adapter, props, emit)` - Main hook over the binding `bindComponent(adapter, VueProfile)`; дженерики не пишутся: инстанс и выходы плагинов — из контракта контекста, `TProps` — из `props`
...
- The binding's `write` still skips values the core already holds (`TProperty.assign` — `Object.is` with the current value): a setter doesn't need to see a value it already holds. `writeAll`, which runs on every `props` change, skips even earlier — every prop whose value is the same as in the previous set.
