...
приводится к типу декларации: типы пропа перебираются в порядке объявления, и
значение даёт первый, который строку принимает, — `String` любую, `Boolean`
любую, `Number` непустую разобранную, `Object` и `Array` — JSON своего вида.
Спор строки и булева решает порядок, как во Vue: `value="abc"` у полей
(`[String, Number, Boolean, Object, Array]`) остаётся строкой, а у Frame
(`[Number, String]`) `width="240"` — число, `width="50%"` — строка. У пропа,
чей первый тип Boolean, действует HTML-семантика: значимо наличие атрибута,
поэтому `disabled="false"` это `true`, а снять флаг можно только его удалением.
Снятый атрибут остальных пропсов, как и строка, которую не принял ни один тип,
приходит `undefined`, и связка возвращает заданный раньше проп к умолчанию
декларации.
