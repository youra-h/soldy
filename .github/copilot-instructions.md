...
Core (бизнес-логика, headless)
  ↓
Provider (контракты, компиляция, Runtime)
  ↓
Setup (конкретные contributions/providers для core и плагинов)
  ↓
Plugins (DOM-логика)
  ↓
...
```
IAccessor<T>              — get()/set?()/subscribe() на одно свойство
  ↑
IAccessorProvider         — getAccessor(IContractProp) → IAccessor | undefined

IEventProvider             — subscribe(event, handler) → (() => void) | undefined

IProvider                  — extends IAccessorProvider, IEventProvider
```

### Контракты (`contract/`)

```ts
IContributionProp {           // Вход: декларация свойства в контрибуции
    name: string
    kind: TPropKind            // 'state' | 'computed' | 'event'
    mutable?: boolean          // по умолчанию: state→true, computed→false
    triggers?: string[]        // события instance, уведомляющие об изменении
}

IContractProp extends IContributionProp {  // Выход: скомпилированное
    mutable: boolean           // всегда нормализован
    ownerId: symbol            // всегда из Contribution.id
}

IContribution {
    id: symbol
    props: IContributionProp[] // без ownerId — компилятор добавляет
    events: string[]
}

IComponentModel {
    props: IContractProp[]     // с ownerId, нормализованным mutable
    events: string[]
}
```
...
- `compileComponent(contributions, userEvents?)` — чистая функция. Нормализует mutable, добавляет ownerId из Contribution.id.

### Runtime (`runtime/`)

- **TRuntime** — принимает `IComponentModel` + `IProvider`. Строит подписки на свойства и события.
- **TAggregateProvider** — композит: `add(IAccessorProvider | IEventProvider)`. Разделяет провайдеры на два массива (accessor + event), делегирует каждому своё.
- **TObservingAccessorProvider** — универсальный провайдер для IComponent. Подписывается на события instance через triggers из IContractProp.
- **types.ts** — `IAccessor`, `IAccessorProvider`, `IEventProvider`, `IProvider`, `TEmitPayload`
...
contributions/
  core/
    component.ts          ← TComponent (rendered, visible, present)
    component-view.ts     ← TComponentView (tag, classes)
  plugins/
    element.ts            ← TElementPlugin
    instance.ts           ← TInstancePlugin

providers/
  plugins/
    element.ts            ← TElementPluginAccessorProvider
    instance.ts           ← TInstancePluginAccessorProvider

models/
  component-view.ts       ← compileComponent([...]) — готовая модель
```
...
- **Имена типов и интерфейсов**: префикс `I` для interface (`IAccessor`, `IContractProp`), `T` для type и class (`TPropKind`, `TRuntime`, `TStateUnit`)
