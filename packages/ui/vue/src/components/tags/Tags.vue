...
export default { ...SetupTags, components: { Button, Icon, Popover, Scroller, TagsItem } }
</script>
...
<template>
	...
	>
		...
		<slot>
			<!--
				Режим `arrows`: ряд уезжает в ленту, и рядом становится её
				вьюпорт. Признак приходит из ядра (`arrows`), а не сравнением
				`overflow === 'arrows'` в разметке: иначе строка режима
				повторилась бы в шести адаптерах.

				Роль ряда стоит на вьюпорте (`rowAria`), а не на корне: внутри
				`role="listbox"` кнопкам листания места нет, а теги оказались бы
				...
				>
					<template #trigger="{ triggerAria, triggerDataset }">
						<!--
							...
							тега, а не символ многоточия текстом: кегль у трёх точек свой
							на каждом шрифте, и подменить их в одном месте было нечем.
