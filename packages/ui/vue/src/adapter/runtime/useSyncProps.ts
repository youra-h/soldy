...
export function useSyncProps(
	 ...
	 * приходит из декларации (у пропа ядра setup собирает его из
	 * `defaultValues` класса), и отсутствующий в разметке `editable` пришёл бы
	 * как `false` — то есть монтирование затирало бы состояние чужого инстанса
	 * своими умолчаниями. Ровно то, ради чего этот инстанс и передают.
	 */
