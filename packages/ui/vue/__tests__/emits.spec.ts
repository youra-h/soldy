 ...
 * — только собственные пропы компонента, — а `useAdapter` эмитит `update:` по
 * событиям связки `adapter.connect()`, куда входят и плагинные. Проп при этом
 * был объявлен: `useProps` строится по поверхности `TSurface.of`, а та собрана
 * из `getProps()`. То есть проп принимался, но его обратная связь считалась
 * незадекларированной.
 *
...
const DESCRIPTORS: Array<[string, () => IComponentDescriptor]> = [
	...
	['ProgressLinear', ProgressLinearDescriptor],
	['Select', SelectDescriptor],
	['Skeleton', SkeletonDescriptor],
	['Slider', SliderDescriptor],
