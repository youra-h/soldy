 ...
 *
 * Модального окна у библиотеки ещё нет, и ради проверки его не заводим:
 * модальным слоем служит `Frame`, которому плагин поставлен снаружи, из
 * `@bundle:create`, с открытостью по `visible`. Внутри окна — `Select`. Его
 * список телепортирован в `body` соседом окна, а пока закрыт, лежит там без
 * номера слоя выше окна: `TFrame` пишет номер только при показе. Здесь
 * проверяется то, ради чего плагин пересчитывает пометки, а не снимает их при
 * открытии: открытый список получает номер выше окна и остаётся доступным
 * скринридеру.
 *
