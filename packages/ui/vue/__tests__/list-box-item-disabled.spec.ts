 ...
 * рисуется.
 *
 * Обмен сверяет значение из разметки с геттером пропа. Пока геттер `disabled`
 * отдавал итог (своё **или** владельца), своё `true`, пришедшее в выключенном
 * списке, совпадало с итогом, обмен считал его повтором и в ядро не писал — и
 * после включения списка элемент оживал, хотя в разметке выключен. Теперь
 * `disabled` отдаёт своё, итог — `resolvedDisabled`, и шаблоны рисуют итог.
 *
