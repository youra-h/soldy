 ...
 *
 * Входы задаются `componentRef.setInput()`, а не полями: поля входов
 * сгенерированный `TButtonSurface` только объявляет, и запись в поле мимо
 * Angular до ядра не дойдёт. `setInput` даёт тот же `ngOnChanges`, что и
 * привязка у родителя.
 */
