 ...
 *
 * Эмиттеры создаются по списку имён, поэтому в типе базы их нет. Тип им даёт
 * сгенерированный наследник `T<Имя>Outputs` (`generated/*.metadata.ts`, поля
 * `declare` с `TOutputEmitter`), и компонент наследует его, а не базу
 * напрямую: строгий шаблон потребителя читает выход как поле класса и без
 * поля привязку `(actionPress)` не компилирует.
 */
