import type { EventLogEntry } from './EventLog.svelte'

/**
 * Конвертирует raw-имя события в Svelte-колбэк-проп:
 * 'change:visible' → 'onChangeVisible', 'element:ready' → 'onElementReady'.
 */
export function toSvelteHandler(eventName: string): string {
	return (
		'on' +
		eventName
			.split(/[-:]/)
...
export function buildEventHandlers(
	...
	for (const eventName of eventNames) {
		handlers[toSvelteHandler(eventName)] = (payload?: unknown) => {
			onLog({
 ...
 * Возвращает функцию отписки — вызывать из $effect.
 */
 ...
 * Пропускает undefined и одинаковые значения.
 */
