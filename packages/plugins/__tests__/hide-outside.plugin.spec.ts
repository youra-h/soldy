 ...
 * есть `open` и шина событий, как в `scroll-lock.plugin.spec.ts`; модальное
 * окно целиком — `ui/vue/__tests__/dialog.spec.ts`.
 *
 * Разметку тест строит сам, а номер слоя (`data-layer`) ставит руками — так
 * его ставит показанный слой (`TLayer`). Проверяется то, ради чего пометки
