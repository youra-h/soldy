 ...
 * Tooltip, модальное окно. Сообщает событием `dismiss`, а владельца, чью
 * открытость ведёт (`property`), закрывает уже после этого — подписчики
 * застают его открытым и знают причину закрытия. Без привязки
 * (`property: null`) только сообщает.
 *
 ...
 * слоёв ведёт `TLayer` — Frame и модальное окно (`data-layer`, номер растёт с
 * каждым показом), и нажатие в панель, чей слой выше слоя своей, мимо не
 * считается. Слой ниже или узел без слоя — мимо: так закрывается вложенный
 * слой нажатием во внешний.
 *
...
export class TDismissPlugin extends TBasePlugin<any, TDismissPluginEvents> {
	 ...
	 *
	 * Закрывает с причиной `outside`: владелец, который принимает запрос
	 * закрытия (модальное окно), решает сам — закрыться, отклонить нажатие
	 * мимо или отдать решение подписчику. `dismiss` при этом уже пришёл:
	 * нажатие мимо было, закроется окно или нет.
	 */
	private _dismiss(event: MouseEvent | FocusEvent): void {
		this.events.emit('dismiss', event)

		this._open?.close('outside')
	}
