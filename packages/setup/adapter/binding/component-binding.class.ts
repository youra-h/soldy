 ...
 *
 * Память может продолжить прошлую связку, когда фреймворк пересобирает
 * контекст в той же жизни компонента: React делает это, заново устанавливая
 * эффекты (StrictMode, `<Activity>`). Новая связка начинает с копии памяти
 * прошлой, иначе её первый набор снова записал бы разметку во внешний `ctrl`.
 * Прошлая связка при этом не меняется.
 */
 ...
 *
 * `previous` — связка прошлого контекста того же компонента, когда фреймворк
 * пересобирает контекст в той же жизни компонента (React, заново устанавливая
 * эффекты). Новая связка продолжает её память входов: проп, который фреймворк
 * с тех пор не менял, не пишется заново. Прошлая связка не меняется, поэтому
 * продолжений от неё может быть несколько — React вправе звать фабрику дважды.
 */
