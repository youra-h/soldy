 ...
 * это делать.
 *
 * Память входов принадлежит компоненту фреймворка, а не контексту: если
 * фреймворк пересобрал контекст в пределах одной жизни компонента (так делает,
 * например, React), новая связка продолжает память прошлой — `bindComponent`
 * с прошлой связкой. Наружу память не выставлена — её читает только связка.
 */
export interface IComponentBinding {
	 ...
	 * внешний `ctrl` получает разметку; у связки, продолжившей прошлую, первый
	 * вызов сверяется с памятью прошлой.
	 */
