/**
 * collectForwardProps — пропсы, которые компонент не съел: они уходят спредом
 * в атрибуты корня (React, Solid, Svelte).
 *
 * Съедает компонент всё, что объявлено в контракте: пропы (и их триггеры),
 * события и слоты. Слоты берутся по дескриптору, имя слота по умолчанию
 ...
 *
 * `plugins` и `ctrl` — пропы самого адаптера, в контракте их нет.
 */
...
export function collectForwardProps<TProps extends object>(
	...
	const consumed = new Set<string>([defaultSlotName, 'plugins', 'ctrl'])
