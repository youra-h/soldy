 ...
 *
 * Не мутирует данные, только делает relay драйверного `change:order` наружу
 * под тем же именем. Драйвер сам знает, какая команда меняет
 * последовательность элементов (вставка, удаление, перемещение, очистка), а
 * какая только их свойства (`update`) — поэтому расширению не нужно ни
 * сравнивать снимки, ни разбирать `change:items`. Используется для
 * DnD-сортировки и других сценариев, где важен порядок элементов.
 *
	...
	implements IExtension<TItem>, IOrderExtension<TItem>
{
	...
	override install(ctx: IExtensionContext<TItem>): void {
		...
		ctx.driver.events.on('change:order', () => {
