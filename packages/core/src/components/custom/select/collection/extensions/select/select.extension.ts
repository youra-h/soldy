 ...
 *
 * Текст выбранного при этом считается здесь — он не про синхронизацию, а про
 * то, что показывать вместо `placeholder`. Держится приватно и наружу не
 * отдаётся: единственное, что с ним делают, — пишут в `owner.field.value`
 * (экземпляр `TInput`, которым в шаблоне показывается поле, в любом режиме, не
 * только `editable`): в `single` это текст выбранного, в `multiple` всегда
 * пусто — там значение в тегах. `owner.field.placeholder` следует тому же
