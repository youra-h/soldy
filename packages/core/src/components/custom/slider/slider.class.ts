	...
	implements ISlider
{
	...
	grab(index: number, fraction: number): boolean {
		...
		if (this.disabledResolved || !isIndexOf(values, index)) return false
	...
	press(fraction: number): boolean {
		...
		if (this.disabledResolved || values.length === 0) return false
	...
	shift(index: number, count: number): void {
		...
		if (this.disabledResolved || !isIndexOf(values, index)) return
