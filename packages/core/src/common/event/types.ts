 ...
 * констрейнт карты событий (`Record<string, (...args: any) => any>`) она
 * удовлетворяет. Та же идиома «пусто», что у `TCollectionEngine` без расширений:
 * дефолт `TExtensions` — `Record<never, IExtension<T>>`.
 *
