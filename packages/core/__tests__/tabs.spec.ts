...
describe('Коллекция табов с TTabsExtension + TActivationExtension', () => {
	...
	it('добавление табов через коллекцию', () => {
		...
		expect(collection.extensions.plain.length).toBe(2)
		expect(collection.extensions.plain.getAll().includes(tab1)).toBe(true)
		expect(collection.extensions.plain.getAll().includes(tab2)).toBe(true)
	})
	...
	it('closeTab удаляет closable элемент и эмитит item:close', () => {
		...
		expect(collection.extensions.plain.length).toBe(1)
		expect(collection.extensions.plain.getAll().includes(tab1)).toBe(true)
		expect(collection.extensions.plain.getAll().includes(tab2)).toBe(false)
	})
	...
	it('closeTab учитывает резолв closable: tab1 не закрывается, tab2 закрывается', () => {
		...
		expect(collection.extensions.plain.length).toBe(3)
		...
		expect(collection.extensions.plain.length).toBe(2)
		expect(collection.extensions.plain.getAll().includes(tab1)).toBe(true)
		expect(collection.extensions.plain.getAll().includes(tab3)).toBe(true)
	})
	...
	it('TTabsItemExtension.close() удаляет таб и эмитит item:close', () => {
		...
		expect(collection.extensions.plain.length).toBe(1)
		expect(collection.extensions.plain.getAll().includes(tab1)).toBe(false)
		expect(collection.extensions.plain.getAll().includes(tab2)).toBe(true)
	})
