...
Режим — **не `readonly`**. `readonly` приходит из `TInputControl` и означает
«значение менять нельзя»: на нём построен `openable`, при нём панель не
открывается вовсе. Select в режиме «только выбор» обязан оставаться
изменяемым, поэтому у режима своё имя — `editable` (`false` — select-only, по
умолчанию). Имя `mode` тоже занято: так называется режим выбора в коллекции, а
в шаблоне оба adapter-контекста сливаются в один объект.

`editable` уже есть в коде (`TSelect.editable`), пока с минимальной логикой:
снимает `readonly` у вложенного `Input` через производный `fieldReadonly`
(`readonly || !editable`) и ставит `aria-autocomplete="none"` — честный сигнал,
что поле принимает текст, но без автодополнения. Ввод никуда не сохраняется:
`value` остаётся текстом выбранной опции. `inputValue`, фильтрация и
«свободное значение» не реализованы — вместе с ними `aria-autocomplete`
станет `list`/`both`, а до тех пор это было бы обещанием скринридеру, которого
нет. В `editable` клавиатура (`TSelectKeyboardPlugin`) не перехватывает
печатные символы, пробел и `Home`/`End` — они принадлежат тексту поля;
стрелки, `Enter`, `Escape`, `Tab` работают как в select-only.
