...
# Библиотечные пакеты: JS бандлом Vite + .d.ts прогоном tsc, см. «Сборка пакетов».
...
каждый пакет выпускает вторым шагом своего `build` и резолвит соседей как
пакеты, то есть по их собранным `.d.ts` (см. «Сборка пакетов»). Отдельного
...
- **Адаптеры Svelte и Angular пока отдают исходники** — их нет ни в очереди, ни
  в стороже: `tsc` не читает `.svelte` вовсе, а библиотека Angular обязана
  собираться в частичной форме (`ng-packagr`). Каждому нужен свой инструмент и
  своя новая зависимость, поэтому они вынесены отдельными задачами.
- **Решения сборки проверяются по конфигу пакета, а не по фабрике.** Своя копия
  ...
  `packages/setup/__tests__/package-builds.spec.ts`: у каждого пакета очереди
  декларации идут после бандла, `paths` конфига сборки не знает соседей и не
  держит `preserveSymlinks`, а бандл — ESM, модуль на модуль, без минификации и
  в `dist`.
...
декларации в один файл сейчас не заводим.
