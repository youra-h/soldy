...
Всё, что открывается поверх страницы, собирается из шести кусков:
...
- **`TOverlayFocusPlugin`** — модель фокуса, общая часть. Запоминает, откуда
  ...
  закрывает оверлей по Escape, гася клавишу `preventDefault`. Возврата нет
  только после `dismiss`: нажатие и уход фокуса мимо — уже выбор
  пользователя, и `TDismissPlugin` сообщает о них **до** закрытия. Панель
  база берёт у него же (`findPanel()`), а без пометки — у наследника: у
  модального окна панель и есть корень. Наследнику остаются ровно две вещи —
  что делает Tab и куда вернуть фокус, когда запомненный элемент пропал.
- **Две стратегии Tab** поверх этой базы, обе с namespace `focus`: модель
  фокуса у компонента одна, а какая — решает его дескриптор.
  `TPopoverFocusPlugin` пропускает Tab дальше по странице, будто панель
  стоит сразу за триггером (см. Popover в «Готовые паттерны»);
  `TModalFocusPlugin` замыкает Tab в панели — с последней остановки на
  первую, Shift+Tab с первой на последнюю, а из панели без остановок Tab не
  уводит вовсе. Запасного возврата «на триггер» у модального нет: открыть
  окно может и кнопка на другом конце страницы, и код без нажатия. Флага
  `modal` в одном плагине поэтому нет — различается не деталь, а стратегия
  целиком.
...
Пять вещей, которые легко забыть:
