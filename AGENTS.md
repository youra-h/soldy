...
совпадает с путём пакета, `version` задана. Версия у всех одна, у корня её нет;
`fixed` — одна группа ровно из библиотечных пакетов; каждое имя из `ignore` —
пакет воркспейса; ни один changeset не объявляет `major`, пока версия `0.x`.
Новый пакет попадает под проверку сам.
...
- **Связка** — `bindComponent(adapter, profile)` на монтирование: стартовое
  ...
  Адаптеру остаётся сказать, куда писать значение и как отдать событие, и
  решить, в какой момент своего цикла это делать. Своих циклов по аксессору у
  адаптера нет.
  ...
  **`writeAll` получает полный набор пропсов**: отсутствующий в нём проп значит
  ...
  дельтой. Первый набор пишет все заданные пропсы: так внешний `ctrl` получает
  разметку. Сторож — `setup/__tests__/binding.spec.ts`, «связка · повторённый
  проп».
...
умолчаниями — ровно то, ради чего инстанс и передают. Angular пишет входы,
заданные при монтировании, в `ngOnInit`: первый `ngOnChanges` приходит раньше
связки, и без этой записи разметка до внешнего `ctrl` не доезжала.
...
вернуть такой проп из разметки к владельцу было нельзя. Нет умолчания у
декларации — сбрасывать не к чему, и значение остаётся: `undefined` такие
сеттеры не принимают (`items`, `mode` фасадов коллекций). Сторож —
`setup/__tests__/binding.spec.ts`, «связка · снятый проп».
