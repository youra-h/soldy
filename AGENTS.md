...
Описание — два объекта, и оба после создания не меняются. Дескриптор
(`TComponentDescriptor`) — своё объявление поверх родительского: плагин того же
класса и слот того же имени встают на место родительских, умолчания пропсов
пересчитываются от итогового `ctor`, декларации заморожены. Определение
плагина (`TPluginDefinition`) — контракт класса и опции того места, где плагин
поставили. Builder и pipeline дескриптору не нужны: его части независимы,
Builder добавил бы изменяемое состояние, pipeline — порядок, которого нет.
...
- **У класса иерархии `TComponent` констрейнт `TEvents` — его собственная
  ...
  инстанса, которую дескриптор несёт в контракте (`IComponentContract`) и из
  которой выводит свои события (`DescriptorEvents`).
...
(`IComponentContract`), до адаптера он доходит типом контекста —
`IAdapterContext<C>`.

Контракт — единственный параметр типа у дескриптора, контекста и `useAdapter`:
что адаптеру нужно знать о компоненте в типах, лежит в нём по ключам
(`instance`, `eventName`, `slots`, `plugins`). Новый факт — новый ключ, а не
ещё одна позиция в каждой сигнатуре. Руками контракт не пишут: его выводят
`defineComponent` и `definePlugin`. Инстанс контекста `createAdapterContext`
сводит из двух источников — `ctor` дескриптора и `ctrl`: адаптер объявляет
`ctrl` интерфейсом ядра (`IButton`), класс дескриптора его реализует, и
контекст обещает интерфейс — под `ctrl` приходит любая его реализация.

Выходы плагинов (см. «Состав пропсов компонента записан дважды») идут в
состояние тем же путём — они лежат в том же контракте
(`C['plugins']['outputs']`). `useAdapter` React, Solid и Svelte отдаёт `state`
типа `TAdapterState<C>`: к свойствам инстанса добавлены выходы того же вида —
снимок, только чтение, необязательный ключ. Граница та же, `toInstanceState`.
У Angular и webc состояние шаблону отдаёт базовый класс, и компонента, который
читает выход, там нет: `state` у них — `TInstanceState` инстанса, выходы
понадобятся вместе с портом Frame или Select.

Vue отдаёт шаблону не `state`, а рефы (`TBinding`: пропы `TProps`, свойства
инстанса и выходы плагинов контракта), и граница у него своя —
...
добавляются к результату без приведения. Дженерики компонент не пишет ни в
одном адаптере: `useAdapter(adapter, props)` берёт инстанс и выходы из
контракта контекста, а `TProps` — из самих `props`. В `TProps` выходы не
кладутся: это не пропсы.
...
- **Один файл на компонент.** Дескриптор объявляет всё, что компонент отдаёт
  ...
  Плагин объявляется так же —
  `definePlugin({ ctor, namespace, contribution: { … } })`, но один раз,
  константой, а не фабрикой: контракт — свойство класса плагина. Дескриптор
  ставит определение как есть (`plugins: [AriaPluginDescriptor]`) или с
  опциями установки — `DismissPluginDescriptor.with({ focusOutside: true })`;
  исходное определение `with()` не меняет, а умолчания пропсов пересчитывает:
...
- **Состав пропсов компонента записан дважды: в contribution и в типе.**
  Рантайм собирает дескриптор (`getProps()` — contribution компонента, его
  предков и плагинов), а типы всех адаптеров выводятся из
  `DescriptorAllProps`: тип пропсов `ctor` (параметр `TProps` класса ядра,
  `IEntity<TProps>`) и контракты плагинов. Это разные факты, поэтому записи
  две: что принимает класс и что рантайм публикует с триггерами. Проп без типа
  шаблон не проверяет, а проп только в типе компилятор пропускает, и в
  разметке он молча становится атрибутом: так жили `dismiss_enabled` у Select
  и `variant` у Skeleton.

  У плагина запись одна — его contribution. `definePlugin` выводит контракт
  плагина сам, типов ему не передают: состав — из contribution (незащищённые
  пропсы — входы, защищённые — выходы, которые плагин вычисляет, а разметка
  только читает), тип значения — у одноимённого свойства класса плагина или у
  `get` декларации, события — карта класса, суженная до `events` и триггеров
  пропсов. Выход — геттер плагина, как выход компонента — геттер инстанса.
  ...
  адаптера тип выходов доходит в контракте контекста, как тип инстанса (см.
  «`any`: где он честный»). Раньше типы плагина писали явными аргументами
  `definePlugin` рядом с contribution, и записей было две; ещё раньше
  `dismiss_ownerAttribute` описывал рукописный тип, который с плагином никто
  не сверял, а `layout_styles` шаблоны читали без типа — проходило это лишь
  потому, что vue-tsc не проверяет атрибуты `<component :is>`.
  ...
  - Незащищённые пропсы рантайма — с ключами `DescriptorAllProps`.
    Коллекционная часть (`SelectCollectionDescriptor`) своих пропсов в типе не
    несёт — класс фасада объявляет `IComponentProps` — и сверяется в строке
    владельца: её пропсы входят в его интерфейс (`ISelectProps`). `ctrl`
    сторож не сверяет: в рантайме проп объявляет `EntityDescriptor`, а тип
    `ctrl?: TInstance` дописывает адаптер. Прочие поля, которые адаптер
    дописывает к типу сверх дескриптора, сторож не видит — так в типах
    адаптеров жил `plugins`, которого в рантайме не было.
...
- **Состав событий записан один раз — в contribution.** Тип событий
  ...
  триггеры пропсов, свои и `extends` (`eventName` контракта). Ровно их адаптер и
...
ListBox, список Select, Popover и будущий Menu выглядят одинаково. Соблазн
...
самостоятельный фокусируемый виджет. Клавиши плагин берёт только с корня и со
строки элемента, на которой фокус остаётся после клика, а клавиши содержимого
слотов — полей и кнопок шапки, подвала и строки — не трогает; сторожат
`setup/__tests__/list-keyboard.spec.ts` и
`playground/vue/browser/keyboard-activation.spec.ts`. В combobox фокус не
имеет права уходить с поля. Вложить готовый ListBox внутрь Select значит
снимать ему `tabindex`, глушить его клавиатурный плагин и перенаправлять
подсветку наружу. Это не переиспользование, а борьба, и она добавляет ListBox
режимы ради чужого компонента — после чего его тесты начинают охранять два
поведения сразу.
...
Выпадающего компонента со своим списком (`DropDown`) не заводим. Произвольное
содержимое у триггера — `Popover`: оверлей (Frame с якорем и закрытием) плюс
связка «триггер ↔ панель» и модель фокуса немодального диалога, которые не
зависят ни от роли содержимого, ни от его клавиатуры. Роль и клавиатура
содержимого остаются за содержимым: список у Select, меню у Menu, у набора
тегов роли нет вовсе.
...
`shiftSize`), `orientation`, `alignment` и `position` у Tabs, `position` у
Label, `indicator`, `contentFit`, `placement`. Такие остаются union'ами ядра.
Булев флаг на месте вида — тоже значение темы: `plain` у CheckBox стал `view`,
флаг не давал теме ни добавить второй вид, ни отказаться от первого.
...
- **`TFrame`** — телепорт, `rendered`/`visible`, стек z-index и его номер в
  `data-layer`.
...
- **`TDismissPlugin`** (namespace `dismiss`) — «нажали мимо». Сам следит за
  ...
  **после** того, как подписчики получили `dismiss`: они застают владельца
  открытым и знают причину закрытия (по ней плагин фокуса Popover не
  возвращает фокус). Опция `focusOutside` (по умолчанию выключена, Select её
  не включает) даёт `dismiss` и на `focusin` мимо — по тому же правилу
  «внутри», аргументом приходит `FocusEvent`. Панель владельца плагин
  находит по его пометке (`findPanel()`) — соседям по набору она нужна,
  а ссылки на телепортированный узел у плагинов нет.
...
поповер в поповере — тоже панели в `body`, соседи панели владельца, а не её
потомки. Поэтому `TFrame` при показе (а созданный видимым — сразу) выдаёт
возрастающий `zIndex` и пишет тот же номер в `dataset` как `data-layer`
(`FRAME_LAYER_ATTRIBUTE`). `TDismissPlugin` считает нажатие и фокус в панели,
чей слой выше слоя своей, нажатием внутри: клик в список Select внешний
поповер не закрывает, а клик во внешнюю панель закрывает вложенный слой —
его слой ниже. Узел без слоя — мимо. Escape закрывает только верхний слой
другим путём: кто обработал клавишу, гасит её `preventDefault` (не
`stopPropagation`), а внешний слой событие с `defaultPrevented` пропускает.
Стека слоёв у плагинов нет — его и ведёт `TFrame`. Сторожат
`setup/__tests__/overlay.spec.ts` («слои», «уход фокуса», «порядок: сообщить,
потом закрыть»), `core/__tests__/frame.spec.ts` (`data-layer`) и
`ui/vue/__tests__/popover.spec.ts` («вложенные слои», «Select внутри»).
...
базы (`TPluginInternalEvents` — `install` и `destroy`) вывод контракта
плагина (`TPluginContractFrom`) снимает с карты плагина, второго списка имён в
setup нет. Сторожит
...
Тип слотов выводится из этого объявления, второй записи у него нет: свои
слоты перекрывают одноимённые слоты `extends`, как и в рантайме. Значение
scope задаётся через `defineType<T>` — из него берётся тип данных слота, и
значение без типа (`String`) не компилируется. Живёт объявление в `setup`,
**не в core**: у ядра понятия слота нет, оно ничего не рендерит.

Раньше рядом с объявлением лежал тип-зеркало `T<Name>Slots`, и с объявлением
его не сверял ни один тест. Зеркала расходились молча: у TagsItem и ListBoxItem
они не дошли до дескриптора, у Tabs и Select в них не было слотов `item-*`, а
наследники без зеркала (Frame, Control) теряли в типах унаследованный
`default`. Сторож вывода — `setup/__tests__/slots.spec.ts`, его проверки типов
ловит «Типы — Setup».
...
| `TFrame`               | `data-layer` — номер слоя показанной панели, тот же, что `zIndex`               |
| `TAnchorPlugin`        | `data-placement` — фактическая сторона панели после flip                        |
| ядро компонента        | своё состояние — `data-open` у `TSelect` и `TPopover`                           |
...
- **Popover** — немодальный Dialog: `role="dialog"` на панели, имя —
  ...
  `trigger` внутри корня: корень служит панели якорем, а нажатие по открытому
  триггеру — нажатие внутри владельца, и панель не мигает. Своего экземпляра у
  содержимого слота нет, поэтому сторону связки оно получает в scope двумя
  наборами: `triggerAria` (`aria-haspopup="dialog"`, а не `true` — `true`
  значит `menu`; `aria-expanded`; `aria-controls` на `id` панели — формула
  одна, в `TPopover`) и `triggerDataset` (`data-selected`: открытый триггер
  выглядит нажатым). Фокус ведёт `TPopoverFocusPlugin`: при открытии — кадром
  позже на первую остановку Tab панели, без неё — на саму панель
  (`tabindex="-1"`); закрытие крестиком, Escape или самим содержимым
  (`open = false`) возвращает фокус на элемент, с которого открыли (`body` и
  отсоединённый — на триггер), закрытие нажатием или фокусом мимо — нет.
  Escape без `defaultPrevented` закрывает и гасится `preventDefault`. Tab
  ведёт себя так, будто панель стоит в документе сразу за триггером: с
  последней остановки корня — в панель, с последней остановки панели — к
  первой остановке после корня с закрытием без возврата (нет её — закрытие,
  Tab не гасится), Shift+Tab с первой остановки панели — на триггер, панель
  остаётся открытой. Остановки плагин восстанавливает по разметке, раскладку
  не проверяет: кандидатов перебирает, пока один не примет фокус. Ловушки
  фокуса, `aria-modal` и `inert` нет. `lazyMount` не монтирует содержимое до
  первого открытия, дальше закрытие только прячет панель. Сторожат
  `core/__tests__/popover.spec.ts`,
  `plugins/__tests__/popover-focus.plugin.spec.ts`,
  `ui/vue/__tests__/popover.spec.ts` и `playground/vue/browser/popover.spec.ts`.
...
смена режима выбора (её Select тоже слушает сам, `change:mode`: `single →
multiple` выбор не трогает, и `change:selection` не приходит),
переименование выбранной опции, закрытие тега и `Backspace` в `multiple` —
пересчитывает `text` или формулу поля, а поле трогает, только пока оно
показывает текст выбранного, то есть совпадает с формулой поля, посчитанной
до пересчёта (у смены режима — с формулой прежнего режима). Так набранное
доживает до выбора или возврата, хотя при серверном поиске список меняется на
каждом ответе. Признак набора — само поле, флага «печатают» и копии
записанного нет, поэтому набранное, совпавшее с формулой (стёртое до пустого
при пустом тексте выбранного), от показа выбранного не отличить. Сторожат
`core/__tests__/select.spec.ts` («набранное в поле и смена списка», «смена
mode на лету») и `setup/__tests__/select-editable.spec.ts`.
...
### Видимое имя контрола — подпись `Label`, вложением

Видимое имя CheckBox, Switch и `RadioGroup.Item` даёт обёртка `Label`: её
корень — `label`, а контрол — его первый labelable-потомок. Клик по тексту
браузер доводит до поля, а текст подписи становится его доступным именем.
Связки через `for` и `id` нет: у радио своего `id` нет вовсе, а искать поле в
DOM ребёнка — операция над чужим узлом. Своего текста у контролов нет, и
заводить его не нужно: подпись — отдельная часть, которую адресует
потребитель. `TAriaPlugin` остаётся для имени без видимого текста.

Цена вложения — имя собирается из всего текста внутри `label`, а HTML
разрешает там только строчную разметку и не разрешает вложенный `label`:

- корни CheckBox и Switch — `span` (`tag` по умолчанию), и всё внутри —
  `span`;
- декор — коробка CheckBox и дорожка Switch — под `aria-hidden`, иначе текст
  слотов иконок и `on`/`off` вошёл бы в имя;
- корень радио — `label`, поэтому в подписи радио рисуется с `tag="span"`.
  Забытый тег ловит `TLabelNestedWarnPlugin` — предупреждением в консоль, как
  `TTabsContentWarnPlugin`;
- описание и ошибку в подпись не кладут: они тоже вошли бы в имя. Для них —
  `aria_describedBy`.

Своего `disabled` у подписи нет: это был бы второй путь к состоянию контрола.
Выключенную подпись тема красит по `:has(input:disabled)` — псевдокласс на
вложенном поле фиксированного тега разрешён (см. «CSS не стилизуется по
`aria-*`»).

Клик по тексту даёт предкам подписи два `click` — свой и браузерный по полю.
Так ведёт себя любой `label`, и `change` поле получает один. Второй клик не
гасится: `stopPropagation` спрятал бы его от всех предков.

Сторожат `ui/vue/__tests__/label.spec.ts` (разметка, одна подпись у поля,
предупреждение о радио без `tag="span"`),
`ui/vue/__tests__/input-control-attrs.spec.ts` (корень по `tag`,
`aria-hidden` декора), `plugins/__tests__/label-nested-warn.plugin.spec.ts` и
`playground/vue/browser/label.spec.ts` — один `change` на клик по тексту,
имя ровно из текста подписи, контрол по центру первой строки, стороны в LTR и
RTL.
...
- Комментарий о корне шаблона Vue пишется внутри корня, первым узлом, а не
  над ним. В разработке Vue оставляет комментарии, и корнем рендера становится
  фрагмент «комментарий + элемент» (компилятор помечает его
  `DEV_ROOT_FRAGMENT`); в продакшене комментарии вырезаны, и корень снова один
  элемент. Расходятся `$el` компонента — в разработке это текстовый якорь
  фрагмента — и `wrapper.element` в `@vue/test-utils`, который смотрит на
  родителя. То же с комментарием перед веткой `v-else` на корне. Сторож —
  `packages/ui/vue/__tests__/template-root.spec.ts`: шаблон каждого компонента
  компилируется с `comments: true`, и у корня рендера этого флага нет.
