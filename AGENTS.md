...
подсвечивается в списке, без скрытия) и `filter` (несовпавшие опции
скрываются — тот же `TFilterExtension`, что доступен и из кода). Ввод поля —
`TSelect.inputValue`: то, что поле показывает, в select-only всегда равно
`text` выбранного, в `editable` расходится с ним, пока пользователь печатает.
Плагин `TEditablePlugin` только пишет `owner.inputValue` по `input`
вложенного поля и передаёт совпадение в публичный
`TSelectKeyboardPlugin.highlightByText` (тот же алгоритм, что и набор по
буквам с клавиатуры); фильтрацию и сброс несёт `TSelectExtension` —
подписывается на `change:inputValue` и в режиме `filter` пишет
`filter.query`, иначе держит фильтр снятым. Поле возвращается к `text`
выбранного на закрытии панели, при выборе опции и при очистке — `value`
при этом не трогается, он остаётся отдельным от того, что напечатано.
`aria-autocomplete`: без `editable` атрибута нет вовсе, с `editable` и
режимом `none` — `"none"`, `search`/`filter` — `"list"`: панель на вводе
открывается и совпадение объявляется через `aria-activedescendant`, то есть
подсказка скринридеру уже есть. В `editable` клавиатура
...
работают как в select-only. «Свободное значение» (ввод, не совпавший ни с
одной опцией) не реализовано — решение за владельцем.
