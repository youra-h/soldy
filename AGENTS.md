...
`view` у ListBox, Accordion и Tags — псевдоним `TButtonView`, а не свой реестр:
...
Рядом со страницей свойств — страница тестов (`/tests/<тема>`, ссылка в
шапке, обратно — на ту же страницу свойств). Слева меню тем (`events`,
`slots`), в теме — блоки сценариев по компонентам, сверху «Запустить все» и
сводка: автоматические прошли/упали, ручные прошли/ждут/упали, красный бейдж
...
Меню тем — тоже **пересечение**: сценарий виден, если у адаптера есть и
компонент, и фикстура.
...
- `playground/shared/__tests__/scenario-runner.spec.ts` — раннер на поддельном
  хосте: итоги, лимит, ожидание ручного, ✓/✗, перезапуск, `runAll`, сводка;
...
- `playground/vue/__tests__/scenarios.spec.ts` — каждая тема открывается,
  ...
  на сценариях из самого теста, ссылка в шапке.
