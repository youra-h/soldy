...
- **Приведение типа, чтобы заткнуть несоответствие.** `as any`,
  ...
  типы не сходятся — неверен контракт, править надо его. В
  `packages/core/__tests__` это стережёт `tests-no-casts.spec.ts` (по AST, не
  по тексту): `any` в любом типе, `as never`, `as unknown as X`, угловое
  приведение `<T>x`, `@ts-ignore` и `@ts-nocheck` роняют тест; честный
  `@ts-expect-error` с пояснением не запрещён.
...
Сторожит `core/__tests__/src-no-casts.spec.ts` (по AST, рекурсивно по `src`):
`as any`, `as never`, `as unknown as X`, `as TEvented<…>`, угловое приведение
`<T>x`, директивы отключения проверки типов. Голый `any` в констрейнте
дженерика (см. абзацы выше) не запрещён — так распознаётся сама
инвариантность, а не костыль. Allow-список пуст: приведения в адаптерах
фасадов item-элементов, в конструкторах item-адаптеров расширений, во
внутренностях движка коллекции и в `TEvented.relay()` сняты сменой контракта, а
не заменой на `TEventSink`. Новая запись — только поштучно и с задачей на
снятие (см. «Временное исключение без срока»).
