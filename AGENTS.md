...
- **Тип выходам даёт тот же генератор**: рядом с массивами имён он пишет
  абстрактный `T<Имя>Outputs` (`@Directive`, по полю `declare readonly` на
  выход, тип — `TOutputEmitter` по фабрике дескриптора и полному имени
  события), и компонент наследует его, а не `TComponentBase`. Строгая
  проверка шаблона потребителя (`strictTemplates` — умолчание новых
  приложений) читает выход как поле класса: без поля привязка
  `(actionPress)` падает с TS7053, а `$event` получает тип поля — первый
  аргумент события ядра, его и шлёт `syncEvents`. Эмиттеры по-прежнему
  ставит `TComponentBase` по массиву имён: поля `declare` в рантайме нет, и
  эмиттер базы оно не затирает. Тип фабрики класс берёт из манифеста, поэтому
  `descriptor` там — реэкспорт из `@soldy-ui/setup`, а не `const`: выведенный
  тип `const` декларации пакета выписать не могут (TS2883, TS7056). Сторож —
  `packages/ui/angular/__tests__/outputs.spec.ts`: по хосту с привязкой
  выхода на компонент экспорта, их шаблоны проверяет «Типы — Angular», а тип
  поля там же сверяет `expectTypeOf` — с `any` шаблон прошёл бы молча.
