...
2. **Сборка** (`assemble/`) — на одно монтирование: инстанс (`ctrl` или
   `ctor`), признак `embedded`, плагины компонента — свои или владельца
   (`IComponentPlugins`), — аксессор и начальные значения пропсов —
   единственная точка инициализации «фреймворк → ядро» для всех адаптеров
   (`applyInitialProps`).
...
| `assemble/`    | Компонент на одно монтирование: инстанс, `embedded`, плагины (состав, набор с `bundle:create`, плагины снаружи), аксессор, начальные значения пропсов. |
...
- **Состав — один список на монтирование** (`assemble/composition.ts`): плагины
  ...
  аксессора, по нему же находятся плагины, поставленные снаружи, — каждый из
  них больше не выясняет состав сам.
- **Чей набор — решает сборка, один раз.** Плагины монтирования —
  `IComponentPlugins` с двумя реализациями: `TOwnPlugins` собирает набор по
  составу, пишет начальные значения пропсов плагинов, ведёт плагины снаружи и
  уничтожает набор; `TSharedPlugins` — набор владельца, на котором работает
  фасад коллекции, и все эти шаги у него пустые. Контекст адаптера о
  развилке не знает: проверки «свой ли набор» нет ни у него, ни у связки.
- **Контракт внешнего плагина сборка читает из `define`**
  (`pluginContractOf`): его записывает `definePlugin` за классом плагина, а
  подключает `TExternalPlugins` набора — когда бы плагин ни встал (см.
  «Внешний плагин: пропсы — `pluginProps`, события — `plugin:event`»).
- **Правило записи свойства — одно, в `TProperty` аксессора**: то же значение
  не пишется, снятый проп возвращается к умолчанию декларации, начальное
  значение, равное умолчанию, ничего не задаёт. Сборка, связка и плагины
  снаружи зовут `initialize`, `assign` и `reset` и своих копий правила не
  держат.
...
- **Регистрации действуют там, где набор создаётся** (`TOwnPlugins`). Пришедший
  в конфиге набор собран по составу своего владельца, и второй раз состав не
  пересматривается: так фасад коллекции, делящий набор с компонентом, не
  получает его дважды.
...
- **Связка** — `bindComponent(adapter, profile)` на монтирование: состояние
  ...
  Своих циклов по аксессору у адаптера нет. Внутри связка — фасад над тремя
  объектами, по одному на направление: `TStateStore` (ядро → фреймворк),
  `TInputWriter` (фреймворк → ядро) и `TEventRelay` аксессора (события наружу
  без повторов). Новое правило кладите в тот, чьё это направление; `pluginProps`
  — не ветка в записи, а свой приёмник входа (`IInputSink`).
...
```ts
// 1. valueOf() отдаёт снимок — TProperty.value аксессора делает val?.valueOf?.() ?? val
...
всех шести адаптерах сбрасывает к умолчанию само свойство аксессора
(`TProperty.reset`, см. «Две поверхности управления»): оно держит декларацию,
