...
целым), `filter` (несовпавшие опции скрываются) и `none` (ввод ни на что не
влияет — отдельная задача разбирает, нужно ли это значение вообще). Всю
...
есть подсказка скринридеру уже есть. В `editable` клавиатура не перехватывает
