...
сборки: она допустима, только если у пакета есть скрипт `build`. Пакет открыт
к публикации: `private` нет, `publishConfig.access` — `"public"`, `files`
непуст, рядом лежат непустой `README.md` и `LICENSE`, совпадающий с корневым.
