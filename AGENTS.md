...
адаптера Vue (`build:types`) резолвят соседей как пакеты, то есть по их
собранным `.d.ts` (см. «Сборка пакетов»). Сборки Angular в CI
...
- **`.d.ts` собирает `tsc -p tsconfig.build.json`** — вторым шагом, потому что
  ...
  доказывает, что опубликованная поверхность самодостаточна.
- **Порядок в корневом `build` — это зависимости, а не алфавит**: `core` →
  `plugins` → `setup` → `icons-material` → `theme-oren`. Сторожит
