...
подсвечивается в списке) и `filter` (пока ведёт себя как `search` — скрытие
несовпавших опций отдельная задача). Реакцию несёт `TEditablePlugin`: слушает
`input` вложенного поля и передаёт совпадение в публичный
`TSelectKeyboardPlugin.highlightByText` — тот же алгоритм, что и набор по
буквам с клавиатуры. Ввод никуда не сохраняется: `value` остаётся текстом
выбранной опции, набранное на закрытии панели сбрасывается. `inputValue` как
проп ядра и «свободное значение» не реализованы — придут вместе с
фильтрацией. `aria-autocomplete`: без `editable` атрибута нет вовсе, с
