...
значит и поддерживать это шестью способами не придётся.
...
плагина (`_returnField`), и вызывают её три повода:
...
- **`focusout`**, когда фокус ушёл и с корня, и с телепортированной панели
  ...
  ничего не меняет;
- **`change:selection`** — выбор сменился, и это не только про закрытие:
  `single` показывает текст выбранного сразу, `multiple` очищает поле после
  каждого выбора (значение там в тегах, а не в поле) — в обоих режимах ввода.

Текст выбранного (`engine.extensions.select.text`) плагин возвращает в поле
прямой записью в DOM — и вот это место дырявое, причём давно: значением
...
ломает, но поле может показывать набранное вместо выбранного.
