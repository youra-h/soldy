...
Режим — **не проп `readonly` в разметке**, а `editable` (`false` —
select-only, по умолчанию); имя `mode` занято режимом выбора в коллекции, а в
шаблоне оба adapter-контекста сливаются в один объект. Но само имя `readonly`
у режима не отобрано — оно перегружено: `TSelect.readonly` — геттер, и
`editable` в нём авторитетен. `editable: true` всегда даёт `readonly ===
false`, `editable: false` всегда даёт `readonly === true`, что бы ни было
передано в проп `readonly` — второй раз объявить `readonly` в контрибуции
Select с другими триггерами нельзя (`TAccessor` бросает на дубликат имени), а
заводить второй проп ради того же смысла — по той же причине, по которой не
заводим `ComboBox` вторым компонентом. Сырой `_readonly` (то, что реально
пришло в проп) не потерян: `openable` проверяет его напрямую, в обход
перегруженного геттера, поэтому полностью запертый Select (`readonly: true`)
не даёт открыть панель, даже когда `editable: true`.

`editable` уже есть в коде (`TSelect.editable`), пока с минимальной логикой:
через перегруженный `readonly` снимает `readonly` у вложенного `Input` и
ставит `aria-autocomplete="none"` — честный сигнал, что поле принимает текст,
но без автодополнения. Ввод никуда не сохраняется: `value` остаётся текстом
выбранной опции. `inputValue`, фильтрация и «свободное значение» не
реализованы — вместе с ними `aria-autocomplete` станет `list`/`both`, а до тех
пор это было бы обещанием скринридеру, которого нет. В `editable` клавиатура
