...
- **Приведение типа, чтобы заткнуть несоответствие.** `as any`,
  ...
  `.svelte`. Пробел один, временный: `packages/plugins` исключён из блока до
  869f2grdv.
...
Исключение временное: `packages/plugins` — до 869f2grdv (плагины ещё шлют свои
события через `as unknown as`). Голый `any` в констрейнте дженерика (см. абзацы
...
**Событие о вычисленном значении объявляется без аргумента.** У значения,
которое адаптер собирает из нескольких источников, ни один источник не равен
результату: `closable` таба это `элемент ?? владелец`, и элемент шлёт
`undefined` как «наследую», а владелец шлёт своё значение даже тогда, когда у
элемента есть собственное и результат не изменился. Поэтому
`change:closable` у item-адаптеров Tabs и Tags объявлен `() => void`, как
`change:active`, `change:order` и `change:selected` у базовых: событие
значит «перечитай геттер», а значение читатель берёт у адаптера. Аргумент в
такой карте — обещание, которого адаптер не выполняет; `relay` его не ловит,
потому что у цели параметров может быть меньше, чем у источника.

Обратный случай — `change:view` у item-адаптера Tags: источник один, аргумент
совпадает с тем, что вернёт геттер, и событие его несёт.
