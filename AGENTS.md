...
# Библиотечные пакеты: JS бандлом Vite + .d.ts прогоном tsc, адаптер Angular —
# одним проходом ng-packagr, см. «Сборка пакетов».
...
своего `build` (адаптер Angular — тем же проходом `ng-packagr`) и резолвит
соседей как пакеты, то есть по их собранным `.d.ts` (см. «Сборка пакетов»).
Отдельного шага деклараций у адаптера Vue больше нет. Адаптер Angular
собирается там же, общим шагом; стенда у него пока нет, и собирать сверх
библиотеки нечего. Линт и форматирование (`prettier --check .`)
...
(у темы, с её вторым выходом, три; у адаптера Angular — один проход своего
инструмента, см. ниже).
...
- **Порядок в корневом `build` — это зависимости, а не алфавит**: `core` →
  ...
  `react`, `solid`, `webc`, `angular`). Сторожит
...
- **Адаптер Svelte пока отдаёт исходники** — его нет ни в очереди, ни в
  стороже: `tsc` не читает `.svelte` вовсе, ему нужен свой инструмент со своей
  новой зависимостью, поэтому сборка вынесена отдельной задачей.
...
- **Решения сборки проверяются по конфигу пакета, а не по фабрике.** Своя копия
  ...
  по инструменту в скрипте `build` (`vite build` или `ng-packagr`), и пакет
  очереди, который не подходит ровно под один, — падающий тест. У общего рецепта
  декларации идут после бандла, а бандл — ESM, модуль на модуль, без
  минификации и в `dist`. У рецепта Angular оба конфига названы в команде,
  `dest` конфига `-p` — `dist`, а `compilationMode` в `tsconfig.build.json` —
  `partial`. У обоих `tsconfig.build.json` объявляет `paths` сам — сторож
  читает файл, а не цепочку `extends`: без ключа пришли бы `paths` пакетного
  конфига, ведущие соседей в исходники. Соседей в `paths` нет,
  `preserveSymlinks` — тоже.
...
декларации в один файл сейчас не заводим. У адаптера Angular их в один файл
сворачивает сам `ng-packagr`, и путей без расширений в его `.d.ts` нет — но он
импортирует соседей, и через них предел действует и на него.
...
Всё, что открывается поверх страницы, собирается из пяти кусков:
...
Четыре вещи, которые легко забыть:
...
**Модальность фона в слой не входит.** Плагины запирают фокус и прокрутку, но
фон не помечают ни `inert`, ни `aria-hidden` — для скринридера он остаётся
доступным. Причина в том, что снимок «все дети `body`, кроме панели» запер бы
и список Select, открытый внутри диалога: скрытые панели `TFrame` лежат в
`body` без номера слоя. Вынесено задачей
[869f5fyc4](https://app.clickup.com/t/869f5fyc4). `aria-modal` плагин тоже не
пишет: это значение, и пишет его в свой `aria` ядро компонента (см.
«Прочее» в «Доступность (a11y)»).
