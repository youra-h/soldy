...
(сторожит `playground/vue/browser/scroller.spec.ts`).
...
- **Tags** — роли по режиму выбора. В `none` набор — `list`/`listitem`: у
  ...
  **В однострочных режимах (`scroll`, `arrows`, `popover`) тег натуральной
  ...
  `wrap`, где одному тегу переносить некуда. **Ряд `scroll` в поле Select —
  без полосы прокрутки**: поле там постоянной высоты, и классическая полоса
  торчала под его нижней рамкой. Листают ряд пальцем, трекпадом и колесом с
  Shift, а где ещё есть теги, подсказывает маска у края, как у ленты. Ширину
  подсказки ведёт сама прокрутка ряда (`animation-timeline: scroll()`), и
  состояния «есть что листать» у ряда нет; отступ прокрутки той же ширины не
  даёт подсказке накрыть крестик под фокусом. Видимая навигация мышью —
