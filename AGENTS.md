...
- **Drawer** — выезжающая панель у края экрана. Своего паттерна APG у неё нет:
  ...
  конца выезда, при `prefers-reduced-motion` перехода нет. «Присутствия» в
  ядре и `<Transition>` в адаптере нет. **Жест** (`swipe`: `none` по
  умолчанию, `handle` — за полосу у края, `panel` — за любое место, кроме
