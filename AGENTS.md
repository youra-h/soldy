...
- **Tooltip** — Tooltip pattern: `role="tooltip"` и `id` на панели, фокус
  ...
  `aria-describedby` на панель он получает в scope (`triggerAria`) — всегда, а
  не только у открытой: панель не размонтируется (открытость — её `visible`),
  и описание есть уже в первой отрисовке и не запаздывает за фокусом. Поэтому
  `lazyMount` у подсказки нет. Когда показывать, решает
