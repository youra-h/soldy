...
- **Описание не анемично.** `TPropSpec` — приспособленец (Flyweight): одна
  ...
  и наследники.
- **Правило записи — одно, в `TLine`:** `write` (`protected` и то же значение
  не пишутся), `reset` (к умолчанию; значим ключ), `accept` (`undefined` —
  «сняли»), `seed` (начальное: `undefined` и равное умолчанию ничего не
  задают). Сборка, обмен и плагины снаружи зовут линию и своих копий правила
  не держат.
...
- **Обмен** — `adapter.connect(profile)` на монтирование отдаёт `TExchange`:
  ...
  **`inputs.full` получает полный набор пропсов**: отсутствующий в нём проп значит
  ...
  что линия и состояние. React, Solid и Svelte отдают набор на каждом проходе,
