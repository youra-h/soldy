...
умолчанию). Постоянный `readonly` у вложенного `Input` в `Select.vue` — деталь
разметки поля, а не проп Select; совпадение имён обманчиво. Имя `mode` тоже
занято: так называется режим выбора в коллекции, а в шаблоне оба adapter-
контекста сливаются в один объект.
