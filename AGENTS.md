...
- **`TLayer`** (`core/src/components/base/layer/`) — слой: телепорт
  ...
  и **`TDialog`** — модального окна, у которого корень сам и есть слой, а
...
- **`TScrollLockPlugin`** (namespace `scrollLock`) — пока владелец открыт,
  ...
  за его панелью остаётся рабочей.
...
- **`TDialogLayoutPlugin`** (namespace `layout`) — раскладка окна без
  координат: `layout_styles` — `z-index` слоя и размер переменными
  `--dialog-width` / `--dialog-height` (незаданный — без переменной, умолчание
  у темы), `layout_backdropStyles` — тот же `z-index` подложки. Переменными,
  а не инлайном: развёрнутое окно и потолок размера по экрану держит тема, и
  инлайн их перебил бы.
...
потомки. Поэтому слой (`TLayer` — Frame и окно) при показе (а созданный
видимым — сразу) выдаёт возрастающий `zIndex` и пишет тот же номер в
`dataset` как `data-layer` (`FRAME_LAYER_ATTRIBUTE`). `TDismissPlugin`
считает нажатие и фокус в панели, чей слой выше слоя своей, нажатием внутри:
клик в список Select внешний поповер не закрывает, а клик во внешнюю панель
закрывает вложенный слой — его слой ниже. Узел без слоя — мимо. Подложка
модального окна несёт номер своего окна (`backdropDataset`): для окна под
ним она — слой выше, и нажатие по подложке верхнего окна нижнее не
закрывает. Escape закрывает только верхний слой другим путём: кто обработал
клавишу, гасит её `preventDefault` (не `stopPropagation`), а внешний слой
событие с `defaultPrevented` пропускает. Стека слоёв у плагинов нет — его и
ведёт `TLayer`. Сторожат `setup/__tests__/overlay.spec.ts` («слои», «уход
фокуса», «порядок: сообщить, потом закрыть»), `core/__tests__/frame.spec.ts`
(`data-layer`), `core/__tests__/dialog.spec.ts` («слой»),
`ui/vue/__tests__/popover.spec.ts` («вложенные слои», «Select внутри») и
`ui/vue/__tests__/dialog.spec.ts` («окно поверх окна», «Select внутри»).
...
(`TCloseReason`; третья, `button`, — у кнопки закрытия в разметке), через
состояние открытости (`plugins/src/custom/overlay/open-state.ts`, метод
`close`). Владельца, который принимает запрос (`ICloseRequestable`, тип-гард
`isCloseRequestable` в `core/src/components/base/layer/`), они закрывают его
`requestClose(reason)`, остальных — записью открытости, как раньше: Popover
и Select о запросе не знают. Решает запрос владелец: `TDialog` при
`dismissible: false` отклоняет `outside` и `escape` без события, остальное
пропускает через отменяемое `close:before` (`TCloseEvent` — `TActionEvent` с
`reason`). Запись открытости из кода и `v-model` запросом не считаются и
`close:before` не шлют — поэтому не `hide:before`, его шлёт и программное
скрытие: родитель, который пишет открытость сам, с окном не разойдётся.
...
- **Dialog** — Dialog (Modal) pattern: `role="dialog"` и `aria-modal="true"`
  ...
  `open`: окно само и есть слой (`TDialog extends TLayer`), и `open` рядом с
  `visible` был бы вторым путём к одному факту. Корень — сама панель, а
