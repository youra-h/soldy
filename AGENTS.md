...
**Строка, которая только рисует чужой элемент, — `presentational`.** У
`Select.Item` `aria` стоит не на `Button`, а на корне-опции (`role="option"`),
и вложенный `<Button tag="span">` там только строка. Сам по себе `TButton` на
таком теге объявил бы себя кнопкой (`role="button"`, `tabindex`), а `TControl`
дописал бы ему `aria-disabled` — второе объявление состояния на потомке без
своей роли. Булев проп `presentational` (только у Button, умолчание `false`)
снимает все три записи: `role` и `tabindex` — в `TButton._syncButtonAria`,
`aria-disabled` — через хук `TControl._announcesState` («объявляет ли элемент
своё состояние сам», по умолчанию `true`). `data-disabled` для темы и нативный
`disabled` в `attrs` проп не трогает: первое красит строку, второе не ARIA. У
Tabs, Accordion, ListBox и Tags проп не ставится: `aria` элемента биндится на
сам `Button`, узел у них один. Хук, как и `_ariaTag`, `TControl` пересчитывает
на `change:disabled` и `change:tag`; наследник, чей хук зависит от своего
пропа, подписывается на его смену сам и пересчитывает после инициализации
пропа — конструктор `TControl` синхронизирует раньше.
...
обычным `input`, как у любого `Input`; `TSelectExtension` на
`change:selection` — текст выбранного, в любом режиме, не только `editable`
(`single` — текст, `multiple` — всегда пусто, значение там в тегах); и
`TEditablePlugin._returnField` — возврат при Escape и уходе фокуса. Значение
меняется по-настоящему, `change:value` срабатывает, и `Input` перерисовывается
сам — раньше на этом месте была прямая запись в DOM, которую забивал ближайший
рендер вложенного `Input`.
...
`change:selection` сюда не входит: текст поля на смену выбора пишет сама
`TSelectExtension` (см. выше), плагину остаётся сбросить то, что относится
только к вводу — набранное и отбор (`filter.query`), иначе выбор в `multiple`
