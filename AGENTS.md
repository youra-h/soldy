...
инвариантность, а не костыль. Allow-список стережёт то, что в эту задачу не
входит: адаптеры фасадов item-элементов коллекций, конструктор item-адаптера
расширения, внутренности движка коллекции и `TEvented.relay()` — везде
приведение снимается только вместе со сменой контракта, а не заменой на
`TEventSink`.
