...
Так же закрыта и карта событий: у каждого уровня фасада `TEvents` сужен своей
картой (`TCollectionComponentEvents`, `TBatchCollectionFacadeEvents`,
`TSelectionCollectionFacadeEvents`, `TOrderItemFacadeEvents`,
`TSelectionItemFacadeEvents`) **и в констрейнте, и в дефолте**: констрейнт
сверяет `relay` в теле самой базы, дефолт — конечные фасады, которые параметр
не передают. С открытым `Record<string, …>` проверка `TEvented.relay` пустая —
проходит любое имя с любым обработчиком. Конечный фасад объявляет карту рядом с
остальными типами (`<component>/collection/types.ts`) и добавляет к карте базы
только то, что релеит сам; имена и аргументы берутся `Pick`-ом с карт
источников, чтобы переименование события ломало компиляцию, а не оставляло
мёртвое имя.
