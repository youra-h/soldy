...
записью перед подпиской, ни первым набором. Начальное состояние фреймворк
берёт снимком `state()` после сборки, дальше в обе стороны идут только
изменения. Понадобилась стартовая запись в одном адаптере — значит, сборка
чего-то не сделала: чинится в сборке, одинаково для всех, а не подстройкой под
цикл фреймворка.
