...
Ссылка пакета на самого себя — тоже (`"@soldy-ui/setup": ["./index.ts"]`): с
`["."]` «Типы — Setup» проверял спеки, которые импортируют пакет по имени, по
его `dist`, а локально тот бывает старым. Цели `paths` сторожит
`packages/setup/__tests__/source-paths.spec.ts`: ни один `tsconfig*.json`
репозитория не ведёт в каталог с `package.json` — ни точной целью, ни
подстановкой в шаблон с `*`. Алиасы `vitest.config.ts` он не читает.
