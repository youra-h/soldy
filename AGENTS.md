...
- **У класса иерархии `TComponent` констрейнт `TEvents` — его собственная
  ...
  инстанса, которую дескриптор несёт в `IAdapterContext<TInstance>` и из
...
- **Состав пропсов записан дважды: в contribution и в типе.** Рантайм собирает
  ...
  а типы всех адаптеров выводятся из `DescriptorAllProps`: тип пропсов `ctor`
  (параметр `TProps` класса ядра, `IEntity<TProps>`) и, у плагина, третий
  аргумент `definePlugin` (`IAriaPluginProps` — только незащищённые пропсы, без
  неймспейса). Это разные факты, поэтому записи две: что принимает класс и что
  рантайм публикует с триггерами. Проп без типа шаблон не проверяет, а проп
  только в типе компилятор пропускает, и в разметке он молча становится
  атрибутом: так жили `dismiss_enabled` у Select и `variant` у Skeleton.
