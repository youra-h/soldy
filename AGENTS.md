...
- **Карта не пересекает `TAnyEvents`.** Это `Record<string, (...args: any) => any>`
  ...
  принимает любое имя с любым обработчиком. Он только констрейнт дженерика —
  тот же приём, что в «События item-адаптера: `any` в констрейнте, точный набор
  в инстанцировании». Корневая карта `TComponentEvents` закрыта: в ней одно
  `bundle:create` (см. «Две поверхности управления»), и карта фасада коллекции
  начинается с неё, как карта любого компонента. Опечатку в имени события
  компонента в `on`, `emit` и правиле `relay` сторожит
  `core/__tests__/component-events-closed.spec.ts`.
