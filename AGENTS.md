...
снимает `readonly` у вложенного `Input` и ставит `aria-autocomplete="none"` —
честный сигнал, что поле принимает текст, но без автодополнения. Ввод никуда
не сохраняется: `value` остаётся текстом выбранной опции. `inputValue`,
фильтрация и «свободное значение» не реализованы — вместе с ними
`aria-autocomplete` станет `list`/`both`, а до тех пор это было бы обещанием
скринридеру, которого нет. В `editable` клавиатура
