...
# Библиотечные пакеты: JS бандлом Vite + .d.ts прогоном tsc, адаптер Svelte —
# одним проходом svelte-package, см. «Сборка пакетов».
...
шаге «Тесты» — как это устроено, см. «Angular-специфика». Шаг «Сборка пакетов»
(`npm run build`) стоит после тестов и перед типами: до него в клоне нет ни
одного `dist`, и этим доказано, что тесты сборки не требуют, — а декларации
каждый пакет выпускает вторым шагом своего `build` (адаптер Svelte — тем же
проходом `svelte-package`) и резолвит соседей как пакеты, то есть по их
собранным `.d.ts` (см. «Сборка пакетов»). Отдельного
шага деклараций у адаптера Vue больше нет. Сборки Angular в CI
...
(у темы, с её вторым выходом, три; у адаптера Svelte — один проход своего
инструмента, см. ниже).
...
- **Порядок в корневом `build` — это зависимости, а не алфавит**: `core` →
  ...
  `react`, `solid`, `webc`, `svelte`). Сторожит
...
- **Адаптер Angular пока отдаёт исходники** — его нет ни в очереди, ни в
  стороже: библиотека Angular обязана собираться в частичной форме
  (`ng-packagr`), это свой инструмент и новая зависимость, поэтому сборка
...
- **Решения сборки проверяются по конфигу пакета, а не по фабрике.** Своя копия
  ...
  по инструменту в скрипте `build`, и пакет очереди, который не подходит ни
  под один, — падающий тест. У общего рецепта декларации идут после бандла, а
  бандл — ESM, модуль на модуль, без минификации и в `dist`. У рецепта Svelte
  вход `src`, выход `dist`, декларации не выключены и идут по
  `tsconfig.build.json`, `svelte.config.*` нет, а в `exports["."]` есть
  условие `svelte`. У обоих `paths` конфига деклараций не знает соседей и не
  держит `preserveSymlinks`.
