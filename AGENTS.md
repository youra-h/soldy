...
# Библиотечные пакеты: JS бандлом Vite + .d.ts прогоном tsc, адаптеры Svelte и
# Angular — одним проходом svelte-package и ng-packagr, см. «Сборка пакетов».
# Тестам и стенду сборка не нужна — они ходят в соседей исходниками
npm run build

# После build: пакеты, которые уезжают в npm, объявляют всё, что импортируют
# (см. «Версии пакетов»)
...
`packages/icons/material/src/index.ts`, — импорты собранных пакетов шагом
«Импорты пакетов объявлены» (см. «Версии пакетов») и сборки. «Типы — Angular» — это
...
писал бы `.js` рядом с исходниками `core`, `setup` и `plugins`. `include` у
этого конфига нет, поэтому под ту же проверку попадают и спеки
`packages/ui/angular/__tests__` вместе с шаблонами своих хост-компонентов, а
`dist` пакета исключён (`exclude`): шаг идёт после «Сборки пакетов», и
декларации сборки попали бы в программу. Рантайм-спеки Angular гоняет
`npm run test:angular` в шаге «Тесты» — как это устроено, см.
«Angular-специфика». Шаг «Сборка пакетов» (`npm run build`) стоит после тестов
и перед типами: до него в клоне нет ни одного `dist`, и этим доказано, что
тесты сборки не требуют, — а декларации каждый пакет выпускает вторым шагом
своего `build` (адаптеры Svelte и Angular — тем же проходом `svelte-package` и
`ng-packagr`) и резолвит соседей как пакеты, то есть по их собранным `.d.ts`
(см. «Сборка пакетов»). Отдельного шага деклараций у адаптера Vue больше нет.
Адаптер Angular собирается там же, общим шагом; стенда у него пока нет, и
собирать сверх библиотеки нечего. Линт и форматирование (`prettier --check .`)
...
`__tests__`). Свой пакет, который тесты импортируют по имени, проверка типов
находит в исходниках, как соседей (см. «Сборка пакетов», «Локальная разработка
сборки не требует»). Приведение вроде `as never` там больше не спрячет
несовпавший контракт компонента или плагина.

**Ошибка или предупреждение в консоли роняет тест адаптера React.** Общий файл
настройки прогона (`packages/ui/react/__tests__/setup.ts`, `setupFiles` в
`vitest.config.ts`) на время каждого теста оборачивает `console.error` и
`console.warn`, вывод печатает как есть, а после теста роняет тест текстами
пойманных вызовов. О проблеме теста — обновлении мимо `act`, дубле ключа,
неизвестном атрибуте — React сообщает только строкой в консоли, и прогон
оставался зелёным. Фильтра по тексту нет: предупреждение React и диагностика
самой библиотеки — одинаково находки, и чинится причина, а не сторож. Тест,
который вывод ожидает, заявляет это шпионом с заглушкой —
`vi.spyOn(console, 'warn').mockImplementation(() => {})`, как `label.spec.tsx`:
заглушка метод под шпионом не зовёт, и сторож такого вызова не видит. Поэтому
сам сторож — обёртка, а не `vi.spyOn`: повторный `vi.spyOn` на тот же метод
вернул бы шпиона сторожа, и заявленный вывод уронил бы тест.
...
- **библиотечные пакеты выкладываются**: `private` у них нет, а
  `publishConfig.access` — `"public"`: у скоупа доступ по умолчанию
  `restricted`. Состав тарболла задаёт `files`, рядом с манифестом лежат
  `README.md` и копия корневого `LICENSE` — npm кладёт оба в тарболл мимо
  `files`. Стенд остаётся `private`. `privatePackages.version: true` всё равно
  обязателен: стенд им не версионируется, но и с ним в `ignore` флаг нужен
  changesets'у, чтобы не пропускать private-пакеты вовсе;
- **чужое содержимое — со своей лицензией.** Код пакетов — MIT, текст — копия
  корневого `LICENSE`. Пакет, который везёт чужое содержимое под другой
  лицензией, называет её в `license` выражением SPDX — `MIT AND Apache-2.0`,
  иначе сканер лицензий увидит одну MIT, — а её текст кладёт рядом с
  манифестом в `LICENSE-<идентификатор>` и вносит в `files`: сам npm кладёт в
  тарболл только `LICENSE`. Так устроен пакет иконок (см. «Пакеты иконок»);
- **соседа пакет объявляет диапазоном от общей версии** — `"^0.1.0"`, и в
  `dependencies`, и в `peerDependencies`. Протокол `workspace:` npm не понимает
  вовсе (`EUNSUPPORTEDPROTOCOL` на `npm install`) — это pnpm и yarn. `"*"` тоже
  не годится: выпуск переписывает диапазон, только когда новая версия из него
  выпала, а из `"*"` не выпадает ничего — в опубликованном пакете он значил бы
  «любая версия ядра». Руками диапазоны не правятся: их поднимает
  `npm run changeset:version` вместе с версией.
...
у каждого непустой `description`, `license` — `MIT` или `MIT AND <идентификатор>…`,
`repository.directory` совпадает с путём пакета, `version` задана. Цели `main`,
`types`, `style` и `exports` (вглубь) — файлы пакета, а цель, которую
игнорирует git, — выход сборки: она допустима, только если у пакета есть
скрипт `build`. Пакет открыт к публикации: `private` нет,
`publishConfig.access` — `"public"`, `files` непуст, рядом лежат непустой
`README.md` и `LICENSE`, совпадающий с корневым. На каждую чужую лицензию из
`license` рядом лежит непустой `LICENSE-<идентификатор>`, внесённый в `files`,
а текста лицензии, не названной в `license`, в `files` нет.
Каждый сосед `@soldy-ui/*` в любом поле зависимостей объявлен `^<его версия>`.
Версия у всех одна, у корня её нет; `fixed` — одна группа ровно из
библиотечных пакетов; каждое имя из `ignore` — пакет воркспейса; ни один
changeset не объявляет `major`, пока версия `0.x`. Новый пакет попадает под
проверку сам.

**Пакет объявляет всё, что импортирует.** Имя пакета из каждого голого
спецификатора в опубликованных файлах стоит в `dependencies` или
`peerDependencies` либо совпадает с именем самого пакета; подпуть
отбрасывается (`solid-js/web` → `solid-js`, `@angular/core/testing` →
`@angular/core`). `devDependencies` не в счёт: потребитель их не ставит. В
репозитории необъявленный импорт не виден — npm поднимает зависимости всех
воркспейсов в общий `node_modules`, — а из пакета, поставленного pnpm или Yarn
PnP, видно только объявленное. Так адаптер Angular импортировал
`@angular/common`, держа его в `devDependencies`. Исключений нет: `node:*` в
опубликованном пакете — такое же нарушение, и чинится оно манифестом пакета,
а не исключением в проверке.

Читается то, что уедет в npm, а не исходники: состав тарболла называет
`npm pack --dry-run` (пакет с `private` не уезжает), тексты берутся с диска.
Импорт — это `import` и `export … from` в любом виде, `import()` в коде и в
типах, `require()`, дополнение чужого модуля и `/// <reference types>` в JS,
декларациях и блоках `<script>` компонентов Svelte. Так виден импорт, который
вставил компилятор (`react/jsx-runtime`), и не виден кодоген, который в пакет
не попадает. Поэтому проверка идёт после сборки —
`npm run check:published-imports`, шаг CI «Импорты пакетов объявлены» сразу за
«Сборкой пакетов», — а не в `__tests__` пакетов: шаг «Тесты» идёт до сборки.
Несобранный пакет её не проходит: целей его точек входа (`main`, `types`,
`style`, `exports`) нет в тарболле. Сторожит `tools/published-imports/`:
правило — на фикстурах, воркспейс — собранным.
...
(у темы, с её вторым выходом, три; у адаптеров Svelte и Angular — один проход
своего инструмента, см. ниже).

```bash
npm run build                                    # все пакеты, по зависимостям
npm run build --workspace=@soldy-ui/core         # vite build, затем tsc
npm run build --workspace=@soldy-ui/svelte       # svelte-package: .svelte как есть + .d.ts
npm run build --workspace=@soldy-ui/angular      # ng-packagr: частичная форма и .d.ts
```

- **JS собирает Vite** по общей фабрике `tools/vite/lib.config.ts`: вход,
  `dist`, `formats: ['es']`, соседи по скоупу — external. Четыре копии одного
  конфига разъехались бы: `external: [/^@soldy\//]` у адаптера Vue пережил
  переименование скоупа и перестал совпадать хоть с чем-нибудь.
- **`.d.ts` собирает `tsc -p tsconfig.build.json`** — вторым шагом, потому что
  у Vite стоит `emptyOutDir` и обратный порядок стёр бы декларации. У конфига
  сборки `paths` пустой (кроме ссылки пакета на самого себя): сосед резолвится
  как пакет, через `node_modules` на свой `dist/index.d.ts`. Этим сборка и
  доказывает, что опубликованная поверхность самодостаточна. У адаптера Vue тем
  же конфигом ходит `vue-tsc`: `.vue` обычный `tsc` не читает, а декларация
  компонента (`Button.vue.d.ts`) лежит рядом с его модулем.
- **Порядок в корневом `build` — это зависимости, а не алфавит**: `core` →
  `plugins` → `setup` → `icons-material` → `theme-oren` → адаптеры (`vue`,
  `react`, `solid`, `webc`, `svelte`, `angular`). Сторожит
...
- **Адаптер Svelte собирает `svelte-package`** (`@sveltejs/package`) — одним
  проходом, бандла у него нет: `tsc` не читает `.svelte` вовсе. `.svelte`
  копируются как написаны, `.ts` транспилируются модуль в модуль
  (`useAdapter.svelte.ts` → `useAdapter.svelte.js`), а декларации
  (`Button.svelte.d.ts` и `.d.ts` модулей) выпускает `svelte2tsx` тем же
  проходом по `tsconfig.build.json` — с пустыми `paths`, как у всех.
  Компоненты заранее не компилируются: скомпилированный компонент привязан к
  `svelte/internal` своей версии, поэтому его, как и руны модулей
  `.svelte.js`, компилирует приложение потребителя. Отсюда `svelte` в
  `peerDependencies` — от 5.29, где появился `{@attach}`, — и условие `svelte`
  в `exports`: по нему плагин Svelte узнаёт библиотеку. Препроцессора и
  `svelte.config.*` у пакета нет: TS и в `<script lang="ts">`, и в разметке
  (`this={state.tag as string}`) Svelte 5 разбирает сам, поэтому `.svelte`
  уезжают теми же, что компилируют тесты пакета, а конфиг подхватили бы разом
  сборка, плагин Svelte в тестах и `svelte-check`. Черновик сборки
  (`.svelte-kit/`) — в `.gitignore`: перенести его можно только конфигом.
- **Адаптер Angular собирает `ng-packagr`** — одним проходом, командой
  `ng-packagr -p ng-package.json -c tsconfig.build.json`. Весь конфиг сборки —
  в двух файлах, которые названы в скрипте; цели в `angular.json` у неё нет,
  там живут опции прогона спеков (см. «Angular-специфика»). Библиотека Angular
  уезжает в **частичной форме** (`compilationMode: "partial"` в
  `tsconfig.build.json`): компоненты несут `ɵɵngDeclareComponent`, и компилятор
  приложения дособирает их под свою версию Angular, а полная форма
  (`ɵɵdefineComponent`) привязана к версии, которой её собрали. Ключ записан
  явно: с конфигом пакета без него `ng-packagr` собирает полную форму. `paths`
  у конфига пустой, как у всех: соседи и в коде, и в декларациях — по имени
  пакета. Формат выхода задаёт **Angular Package Format**, и «модуль на модуль»
  к нему неприменим: один FESM (`dist/fesm2022/soldy-ui-angular.mjs` — ESM, без
  минификации) и декларации, свёрнутые в один файл
  (`dist/types/soldy-ui-angular.d.ts`), — такой пакет и ждёт Angular CLI
  потребителя. Соседи лежат в `dependencies`, а `ng-packagr` пропускает там
  только `tslib`, поэтому в `ng-package.json` —
  `allowedNonPeerDependencies: ["^@soldy-ui/"]`.
- **У манифеста Angular нет `exports`.** `ng-packagr` строит свой манифест
  выхода из нашего и `exports["."]` пишет сам: наши условия `types` и
  `default` для него — конфликт, строку он перетирает. Точки входа — `main` и
  `types`, ровно те файлы, что его манифест называет в `exports["."]`.
  Выкладывается пакет, как все, из своего каталога: второй манифест
  (`dist/package.json`) и копии README и LICENSE, которые `ng-packagr` кладёт в
  `dist` для выкладки оттуда, `files` исключает.
- **Решения сборки проверяются по конфигу пакета, а не по фабрике.** Своя копия
  конфига рядом с пакетом расходится с фабрикой молча: у адаптера Vue такая
  копия держала формат `cjs` и складывала выход в `lib` мимо `dist`,
  объявленного в манифесте. Сторожит
  `packages/setup/__tests__/package-builds.spec.ts`. Рецепт пакета он узнаёт
  по инструменту в скрипте `build` (`vite build`, `svelte-package` или
  `ng-packagr`), и пакет очереди, который не подходит ровно под один, —
  падающий тест. У общего рецепта декларации идут после бандла, а бандл — ESM,
  модуль на модуль, без минификации и в `dist`. У рецепта Svelte вход `src`,
...
- **Только ESM, без CJS.** Реестры плагинов и расширений узнают тип через
  `instanceof` («Плагины и расширения снаружи»), а двухформатный пакет даёт в
  одном приложении две копии класса — реестр молча перестал бы узнавать
  компонент.
- **Модуль на модуль, а не один файл** (`preserveModules`). Слепленный пакет
  теряет tree-shaking: приложение с одной кнопкой получало 420 KiB вместо
  224 KiB — вместе с дескрипторами Select, Tabs и Accordion. Сторожит второй
  тест `packages/setup/__tests__/tree-shaking.spec.ts`. Бандлер здесь нужен не
  ради одного файла, а ради спецификаторов: исходники импортируют соседние
  модули без расширений и папками (`export * from './components'`), а
  собранный `tsc`-ом пакет с такими путями в Node ESM не завёлся бы вовсе.
- **Минификации нет.** Библиотеку минифицирует сборщик приложения, а своя
  только коверкает имена классов, которыми подписаны ошибки сборки набора
  («TAriaPlugin уже входит в состав TButton»).

**Локальная разработка сборки не требует.** Тесты, стенд и проверки типов
ходят в соседей исходниками — алиасами в `vitest.config.ts` и `paths` в
`tsconfig.json`. Правка в ядре обязана быть видна без промежуточного билда,
...
берёт в программу `__tests__`, в `paths` есть ссылка на себя — в исходники:
`"@soldy-ui/vue": ["./src/index.ts"]`. Без неё имя уходило через
`node_modules` в манифест: без сборки «Типы — Vue» падали на «модуль не
найден», а со старой сборкой молча сверяли тесты с устаревшими типами.
Каталог с манифестом (`"."`, как было у setup) ведёт туда же, только
незаметно: пока `dist` нет, TypeScript откатывается на `index.ts` рядом.
Сторожит `packages/setup/__tests__/package-builds.spec.ts`: имя пакета из его
тестов TypeScript разрешает мимо манифеста и мимо `dist`, собран пакет или
нет.

**Предел, оставленный осознанно:** `.d.ts` — дерево с импортами без
расширений. У потребителя с `moduleResolution: bundler` (Vite, Next, любой
сборщик) они разрешаются, с `node16`/`nodenext` — нет (TS2834). Сворачивать
декларации в один файл сейчас не заводим. У адаптера Svelte без расширений и
сам JS: бандлера у него нет, и `export * from './components'` уезжает как
написан. Без сборщика с плагином Svelte пакет не подключить вовсе, а
сборщики такие пути разрешают; webpack 5 в строгом ESM — с
`resolve.fullySpecified: false`. Переписывать исходники под `.js` ради него
не стали. У адаптера Angular декларации в один файл сворачивает сам
`ng-packagr`, и путей без расширений в его `.d.ts` нет — но он импортирует
соседей, и через них предел действует и на него.
...
1. **Вся изменяемая память слоя — ячейки (`TCell`).** Значение, правило «то же
   самое» и подписчики; контракт как у стора Svelte: `subscribe` сразу отдаёт
   значение, `listen` — только изменения. Память «что задал фреймворк», снимок
   состояния и мешок `pluginProps` — ячейки, флагов «задан / не задан» нет.
2. **Срез вертикальный, по свойству.** Линия (`TLine`) — описание пропа
   (`TPropSpec`) + владелец + имя во фреймворке. Правила записи живут в ней по
   одному разу — для сборки, обмена и плагинов снаружи.
3. **Все, кто владеет свойствами, — участники одного вида (`TMember`)**:
   инстанс, плагин дескриптора, плагин снаружи и сама связка
...
- **Описание не анемично.** `TPropSpec` — приспособленец (Flyweight): одна
  запись на тип, которую делят все монтирования, без владельца и памяти, но с
  поведением — `read(owner)` и `assign(owner, value)`. Свои `get`/`set`
  объявления больше нигде не разбираются. Умолчание — от класса владельца:
  `rebase` и `withDefault` возвращают новое описание, исходное делят родитель
  и наследники.
- **Правило записи — одно, в `TLine`:** `write` (`protected` не пишется),
  `reset` (к умолчанию; значим ключ), `accept` (`undefined` — «сняли»), `seed`
  (начальное: `undefined` и равное умолчанию ничего не задают). Сборка, обмен
  и плагины снаружи зовут линию и своих копий правила не держат.
- **То же ли это значение, решает сеттер, а не линия.** Повтор рендера гасит
  ячейка входа, а дошедшее до линии пишется сеттером, и он сверяет со своим
  записанным значением (см. «Контракт границы core → ui»). С геттером линия не
  сверяет: у свойства с резольвером он отдаёт итог, и значение, равное итогу,
  но не своему, пропадало — `disabled` пункта, выключенного разметкой в
  выключенном списке, толщина Spinner, равная автоматической, `value` Slider,
  равное прижатому. Каждое такое свойство обходили своим публичным `resolved*`
  рядом со старым именем — три обхода одного правила.
- **Три способа доставки пропсов различаются только тем, что считать
  изменением:** `TInput.offer` (полный набор и одиночный проп — смена
  значения) и `TInput.push` (дельта — само наличие ключа: `el.open = false` у
  Web Components — команда, а не повтор рендера). Что делать с изменением,
  решает линия.
- **Поверхность и линия связаны ссылкой, а не строкой.** `TSurface.entryOf`
  находит запись по объекту имени (`TName`): он создаётся один раз на
  объявление и переживает пересчёт умолчаний, поэтому описание наследника
  находит ту же запись. Сшивки «поверхность × свойства по строковому ключу»
  нет.
- **События без повторов — по построению.** `TSurface.events` держит их по
  полному имени, то есть по паре «источник, сырое имя»; `TEventRelay` заводит
  один обработчик на маршрут и в нём же, сразу после события ядра, шлёт
  событие привязки (`update:<проп>`) — порядок задан кодом, а не очерёдностью
  подписок на шине.
- **Чей набор — решает сборка, один раз.** `TOwnBundle` ставит плагины
  дескриптора, заводит `TExternalPlugins`, после начальных значений ставит
  плагины реестра и объявляет набор на микрозадаче, а при `destroy()`
  отвязывает узел до уничтожения набора. `TSharedBundle` — набор не наш (чужой
  или никакого): участники — плагины дескриптора, найденные в наборе, всё
  остальное пусто. Проверки «свой ли набор» нет ни у контекста, ни у обмена.
- **`pluginProps` — не частный случай.** У `TExternalPlugins` есть обычное
  свойство `pluginProps`, и обычная линия пишет в него обычным правилом. На
  каждый пришедший плагин с контрактом он заводит `TExchange`: мешок — это его
  «пропсы сборки» и «полный набор», конверт `plugin:event` — его приёмник
  событий. «Ключ пропал — проп вернулся к умолчанию» нигде не написано
  отдельно.
- **Путь внешнего плагина один — порядком трёх шагов.** `TExternalPlugins`
  подписывается на набор после плагинов дескриптора и до плагинов реестра;
  реестр ставит свои обычным `bundle.use`, и они приходят тем же событием
  `use`, что и плагин из обработчика `bundle:create`.
- **Пропсы связки** (`ctrl`, `embedded`, `pluginProps`) — имена опций самого
  `createAdapterContext`. `ctrl` и `embedded` читаются один раз при сборке и в
  обмене не участвуют; `pluginProps` принимает связка.
- **Начальные значения — только в сборке** и в общем формате имён
  (`CommonProfile`): имя пропа одно во всех фреймворках, профиль адаптера
...
- **Пакет объявлен `"sideEffects": false`** — иначе вызовы `defineDescriptor` /
  ...
  пакете поэтому запрещён. Единственный эффект импорта — запись контракта
  плагина в `definePlugin`; чтобы модуль с ней не выбросило, плагин с
  контрактом ставят снаружи определением: `usePlugins(TButton,
[AnchorPluginDescriptor])`. Сторож — `__tests__/tree-shaking.spec.ts`.

**Что видит адаптер.** `adapter.connect(Profile)` отдаёт `TExchange`:

- `state.subscribe(listener)` / `state.getSnapshot()` — ядро → фреймворк.
  Подписка сразу отдаёт каждое свойство тем же вызовом, что и триггер; снимок
  — тот же объект, пока ни одна ячейка не сменилась; обе функции передаются
  без `this`;
- `inputs.full(props)` / `inputs.delta(changes)` / `offer(v)` у входа (`for (const input of inputs)`)
  — фреймворк → ядро;
- `events.listen(sink)` — события в именах фреймворка;
- `forward(props)` — пропсы, которые компонент не съел.

Адаптер решает, куда положить значение, как отдать событие и в какой момент
своего цикла это сделать. Когда и в каком порядке — общий слой.
...
класса, слот того же имени и проп того же имени встают на место родительских,
умолчания пропсов пересчитываются от итогового `ctor`, описания заморожены. Проп
при этом переобъявляется, а не переписывается: наследник пишет только то, что
меняет (см. «Переобъявление пропа»). О двух событиях с одним полным именем и о
пропе, который столкнулся с пропом плагина, дескриптор сообщает в консоль
(`console.error`) и работает дальше — дубль на совести автора, а плагины
разводит неймспейс. Определение плагина
(`TPluginDefinition`) — контракт класса и опции того места, где плагин
...
списка. Из разметки оно доходит до сеттера тем же путём: обмен с геттером не
сверяет (см. «Контракт границы core → ui»).
...
`batch.patch` и число `change:disabled` у элементов. Запись из разметки —
`ui/vue/__tests__/list-box-item-disabled.spec.ts`.

### `size` и `variant` элемента: владельца, и только его

Размер и вид элемента коллекции диктует владелец — как его ни наполняй.
Механизм тот же, что у `disabled`, но правило другое: не «своё или владельца»,
а «всегда владельца». Резольвер возвращает `owner.size`/`owner.variant`, своё
значение элемента лежит в `rawValue` и ни на что не влияет — ни на классы, ни
на `getProps`. Правило одно на все коллекции — `bindStyleToOwner`,
`notifyOwnerSize` и `notifyOwnerVariant` в `base/stylable/owner-style.ts`;
расширения `item.size`/`item.variant` не пишут.

Продуктовый выбор владельца: «опасный» пункт другого вида — возможность,
которой пока никто не пользуется, и делать её нужно вместе с компонентом,
которому она понадобится. Поэтому в разметке `size` и `variant` у `*.Item` не
задать: у всех шести элементов они переобъявлены защищёнными (фрагмент
`OWNER_STYLE_PROPS`, см. «Переобъявление пропа»). Из класса ядра, из
`IStylableProps` и из типа данных `items` они никуда не делись — значение
приходит оттуда, а резольвер всё равно отдаёт владельцево.

Отсюда второй контракт `TStateUnit`: **`notify(oldValue)` принимает прежний
итог**. Внешние данные резольвера сменились мимо state-unit, и настоящую пару
«было/стало» знает только тот, кто их менял, — у владельца это
`payload.oldValue` его `change:size`. Совпал с текущим — события нет. Без
аргумента подписчик получил бы `oldValue === newValue`, а по `oldValue`
`TStylable` снимает старый класс `--size-*`: на элементе остались бы два.
Сеттеры `TStylable.size`/`variant` сравнивают со своим (`rawValue`), как
`TControl.disabled`, — иначе своё значение не доехало бы даже до `rawValue`.
...
- **Переобъявление пропа: наследник пишет только то, что меняет.** Поверхность
  ...
  элемента коллекции снимает вход, а тип и триггеры остаются объявленными один
  раз, у того, кто проп завёл (`StylableDescriptor`). Место в списке остаётся
  родительским: порядок пропсов публичен, его хранят сгенерированные
  метаданные Angular. У событий такого правила нет: имя фактов не несёт, и
  повтор — дубль, о котором дескриптор сообщает.

  **Защищённое имя уходит и из типа пропсов.** `protected` значит «значение
  вычисляет владелец, разметка только читает», и входа у разметки нет ни в
  рантайме (`exportProps` поверхности), ни в типе: `DescriptorProps` вычитает
  имена, которые дескриптор объявил защищёнными (`protectedName` контракта).
  Обе записи состава меняет одно объявление, и сторож `descriptor-props-types`
  ...
  у `RadioGroup.Item` — ещё `view` и `name`. Чего переобъявлением не делают:
  не убирают проп из класса ядра, из его интерфейса пропсов и из типа данных
  `items` — из данных и от владельца значение по-прежнему приходит, — и не
  «прячут» проп, который просто не нужен: защищённый проп остаётся в
  состоянии, его читают шаблон и тема. Сторож —
  `packages/setup/__tests__/prop-redeclare.spec.ts`.
...
**Иконки чужие.** SVG в `src/` — Material Symbols от Google, стиль Rounded
(fonts.google.com/icons), под лицензией Apache-2.0; код пакета — MIT. Поэтому
`license` у пакета — `MIT AND Apache-2.0`, текст Apache лежит в
`LICENSE-Apache-2.0` (см. «Версии пакетов»), а README называет источник.
Apache-2.0 требует пометить изменённые файлы: шапку `src/index.ts` — источник,
лицензию и что изменено — пишет генератор, и пометка `@license` доносит её до
`dist/index.js`, где обычный комментарий сборка теряет. Новую иконку берут из
того же набора и того же стиля: шапка, README и `license` говорят только о нём.
...
`playground/vue/browser/select-tags.spec.ts`).
...
- Фикстура `__tests__/theme.d.ts` в `core`, `ui/vue`, `ui/react`, `ui/webc` и
  ...
  `variant: 'normal'` в умолчании ядра и `view="plain"` в шаблоне Vue или
  Angular прошли бы типы. Тесты задают значения только из фикстуры.
...
Всё, что открывается поверх страницы, собирается из этих кусков:

- **`TLayer`** (`core/src/components/base/layer/`) — слой: телепорт
  (`target`), видимость (по умолчанию скрыт) и место в стеке z-index с его
  номером в `data-layer`. Общая база **`TFrame`** — слой с координатами
  (`x`/`y`/`width`/`height`), их раскладывает `TFrameLayoutPlugin` инлайном, —
  и **`TModalLayer`** — модального слоя, у которого корень сам и есть слой, а
  место и размер раскладывает тема. **Стек один на все слои** — статика
  `TLayer` (`baseZIndex`, `nextZIndex()`): со своим счётчиком у окна список
  Select, открытый из него, получил бы тот же номер и встал бы под окно.
  Здесь же контракт закрытия пользователем (см. ниже, «Закрытие
...
- **`TAnchorPlugin`** (namespace `anchor`) — привязка к чужому элементу:
  ...
  привязка — за слежение за посторонним элементом. Выбор потребителя —
  `placement`: сторона `top` или `bottom` и выравнивание по якорю — по
  началу (`-start`), по концу (`-end`) или по центру, значением без суффикса
  (`top`, `bottom`, как у Floating UI; центр — умолчание Tooltip). Поверх
  него плагин сам решает **flip** (переключает `top`/`bottom`, если на
  выбранной стороне панель не влезает по высоте окна, а на противоположной
  места больше — иначе остаётся на стороне потребителя; выравнивание, центр в
  том числе, flip не трогает) и **shift** (сдвигает `x` внутрь окна, чтобы
  панель не вылезала за левый и правый край, — центр тоже; шире окна —
  ...
  `-end` — левый, центр от направления не зависит. Фактическая сторона после
  flip уходит теме через `data-placement` на самом Frame — с выравниванием
  потребителя, у центра без суффикса (`top`, `bottom`). Размер якоря и панели
  плагин узнаёт без scroll/resize окна — на обоих висит свой
  `ResizeObserver`, поэтому позиция пересчитывается и когда меняется только
  их размер (пропал тег, вырос Popover). Flip выключается
  `anchor_flip: false` (по умолчанию включён): панель держит сторону
  потребителя, даже если там не влезает, — так Select выражает свои
  `placement: 'top'` и `'bottom'`. Это сторона без flip, а не центр: якорю
  Select отдаёт `top-start` и `bottom-start`. Shift от выключателя не
  зависит.
- **`TDismissPlugin`** (namespace `dismiss`) — «нажали мимо». Сам следит за
  ...
  **после** того, как подписчики получили `dismiss`: они застают владельца
  ...
  возвращает фокус). Закрывает с причиной `outside` — запросом, если
  владелец его принимает (см. «Закрытие пользователем — запрос»). Опция
  `focusOutside` (по умолчанию выключена, Select её не включает) даёт
  `dismiss` и на `focusin` мимо — по тому же правилу «внутри», аргументом
  приходит `FocusEvent`. Панель владельца плагин находит по его пометке
  (`findPanel()`) — соседям по набору она нужна, а ссылки на
  телепортированный узел у плагинов нет.
- **`TOverlayFocusPlugin`** — модель фокуса, общая часть. Запоминает, откуда
  открыли, кадром позже уводит фокус на первую остановку Tab в панели (её
  нет — на саму панель, `tabindex="-1"`), при закрытии возвращает его назад и
  закрывает оверлей по Escape с причиной `escape`, гася клавишу
  `preventDefault` — и тогда, когда владелец запрос отклонил: клавиша
  адресована верхнему слою. Панель база берёт у `TDismissPlugin`
  (`findPanel()`), а без пометки — у наследника: у модального окна панель и
  есть корень. Наследнику остаются три вещи — что делает Tab, куда вернуть
  фокус, когда запомненный элемент пропал, и что делать после `dismiss`.
- **Две стратегии фокуса** поверх этой базы, обе с namespace `focus`: модель
  фокуса у компонента одна, а какая — решает его дескриптор.
  `TPopoverFocusPlugin` пропускает Tab дальше по странице, будто панель
  стоит сразу за триггером (см. Popover в «Готовые паттерны»), а после
  `dismiss` фокус не возвращает: страница за панелью рабочая, и
  пользователь уже там, куда нажал. `TModalFocusPlugin` замыкает Tab в
  панели — с последней остановки на первую, Shift+Tab с первой на последнюю,
  а из панели без остановок Tab не уводит вовсе — и возвращает фокус при
  любом закрытии: страница за окном недоступна. Запасного возврата «на
  триггер» у модального нет: открыть окно может и кнопка на другом конце
  страницы, и код без нажатия. Флага `modal` в одном плагине поэтому нет —
  различается не деталь, а стратегия целиком.
- **`TScrollLockPlugin`** (namespace `scrollLock`) — пока владелец открыт,
  страница под ним не прокручивается. Открытость плагин ведёт сам
  (`property`/`enabled`), как `TDismissPlugin`, а не `watch` в адаптере.
  Ставится адресно, тому, кто перехватывает работу со страницей: немодальному
  оверлею — Popover, списку Select — прокрутку фона запирать нельзя, страница
  за его панелью остаётся рабочей. Выезжающая панель привязывает его не к
  `visible`, а к производному значению ядра `locksScroll` — открыта и не
  `contained`: внутри контейнера панель модальна, но страница за контейнером
  та же, и флага для этого в плагине нет. Запирает он сразу, а отпускает —
  когда корень доиграл переходы закрытия (см. «Замок прокрутки — счётчик на
  документ» ниже).
- **`THideOutsidePlugin`** (namespace `hideOutside`) — пока владелец открыт,
  фон под ним спрятан от скринридера: соседи панели и соседи её предков
  вплоть до `body` получают `aria-hidden="true"`. Открытость плагин ведёт сам
  (`property`/`enabled`), как `TScrollLockPlugin`; панель берёт у
  `TDismissPlugin` (`findPanel()`), а без пометки — корень, как
  `TModalFocusPlugin`. Ставится адресно, модальному: у немодального оверлея
  страница за панелью остаётся рабочей, и скринридер обязан её видеть.
- **Раскладка модального слоя** (namespace `layout`) — без координат:
  `layout_styles` — `z-index` слоя и размер переменными (незаданный — без
  переменной, умолчание у темы), `layout_backdropStyles` — тот же `z-index`
  подложки. Переменными, а не инлайном: развёрнутое окно и потолок размера по
  экрану держит тема, и инлайн их перебил бы. Расчёт один — абстрактный
  `TModalLayoutPlugin`, — а имена переменных читает CSS блока, поэтому у
  наследников они свои: `TDialogLayoutPlugin` — `--dialog-width` /
  `--dialog-height`, `TDrawerLayoutPlugin` — `--drawer-width` /
  `--drawer-height`. Свои переменные сверх размера наследник отдаёт точкой
  `_panelVariables`: у окна это отступ от краёв экрана
  (`--dialog-offset-top`, `-bottom`, `-start`, `-end`, см. Dialog в «Готовые
  паттерны»), у панели своих нет. Пересчёт идёт на каждый повод, а
  `change:styles` — только когда стили сменились по содержимому.
- **`TDrawerSwipePlugin`** (namespace `swipe`) — жест выезжающей панели:
  смахнуть к её краю, чтобы закрыть (см. Drawer в «Готовые паттерны»). Сдвиг
  во время жеста — операция над узлом: плагин пишет его в корень переменной
  `--drawer-swipe` (px со знаком оси окна), и через обмен на каждом кадре он
  не ходит. В ядро идут только значения — признак «тянут» (`beginSwipe` /
  `endSwipe`, `data-swiping`) и закрытие запросом с причиной `swipe`.

Семь вещей, которые легко забыть:
...
потомки. Поэтому слой (`TLayer` — Frame, окно и выезжающая панель) при
показе (а созданный видимым — сразу) выдаёт возрастающий `zIndex` и пишет
тот же номер в `dataset` как `data-layer` (`FRAME_LAYER_ATTRIBUTE`).
`TDismissPlugin` считает нажатие и фокус в панели, чей слой выше слоя своей,
нажатием внутри: клик в список Select внешний поповер не закрывает, а клик
во внешнюю панель закрывает вложенный слой — его слой ниже. Узел без слоя —
мимо. Подложка модального слоя несёт номер своей панели (`backdropDataset`):
для окна под ним она — слой выше, и нажатие по подложке верхнего окна нижнее
не закрывает. Escape закрывает только верхний слой другим путём: кто
обработал клавишу, гасит её `preventDefault` (не `stopPropagation`), а
внешний слой событие с `defaultPrevented` пропускает. Стека слоёв у плагинов
нет — его и ведёт `TLayer`. Сторожат `setup/__tests__/overlay.spec.ts`
(«слои», «уход фокуса», «порядок: сообщить, потом закрыть»),
`core/__tests__/frame.spec.ts` (`data-layer`), `core/__tests__/dialog.spec.ts`
(«слой»), `ui/vue/__tests__/popover.spec.ts` («вложенные слои», «Select
внутри»), `ui/vue/__tests__/dialog.spec.ts` («окно поверх окна», «Select
внутри») и `ui/vue/__tests__/drawer.spec.ts` («внутри модального окна»).

**Закрытие пользователем — запрос, а не запись.** Нажатие мимо и Escape
плагины слоя передают владельцу с причиной — `outside` и `escape`
(`TCloseReason`; `button` — у кнопки закрытия в разметке, `swipe` — у жеста
выезжающей панели), через состояние открытости
(`plugins/src/custom/overlay/open-state.ts`, метод `close`). Владельца,
который принимает запрос (`ICloseRequestable`, тип-гард `isCloseRequestable`
в `core/src/components/base/layer/`), они закрывают его
`requestClose(reason)`, остальных — записью открытости, как раньше: Popover
и Select о запросе не знают. Решает запрос владелец: модальный слой
(`TModalLayer` — окно и выезжающая панель) при `dismissible: false`
отклоняет `outside` и `escape` без события, остальное — кнопку и жест, его
потребитель включает сам, — пропускает через отменяемое `close:before`
(`TCloseEvent` — `TActionEvent` с `reason`). Запись открытости из кода и
`v-model` запросом не считаются и `close:before` не шлют — поэтому не
`hide:before`, его шлёт и программное скрытие: родитель, который пишет
открытость сам, с окном не разойдётся.

**Нажатие мимо модального окна фокус не забирает.** Окно закрывается ещё на
`pointerdown`, и фокус сразу возвращается туда, откуда открыли, но действие
по умолчанию у `mousedown` того же нажатия унесло бы его на `body`. А окно,
которое нажатие не закрыло (`dismissible: false`), осталось бы без фокуса, и
Tab с `body` повёл бы на страницу под ним. Поэтому `TModalFocusPlugin` на
`dismiss` гасит `mousedown` этого нажатия: сам, если нажатие решил он (перо
графического планшета), иначе — ближайший следующий; ожидание снимают
`click` и `pointercancel`. jsdom действия `mousedown` не выполняет — перенос
фокуса сторожит `playground/vue/browser/dialog.spec.ts`, погашенное событие —
`plugins/__tests__/modal-focus.plugin.spec.ts`.

**Замок прокрутки — счётчик на документ, а не флаг у плагина.** Слоёв бывает
несколько — диалог, из него выезжающая панель, — и каждый запирает прокрутку
сам; флагом закрытие верхнего вернуло бы прокрутку, пока нижний ещё открыт.
Поэтому замок один на документ
(`plugins/src/custom/overlay/scroll-lock/lock.ts`), и стиль возвращается,
когда его отпустил последний слой, — ровно те **инлайновые** значения, что
замок перебил. `overflow: hidden` ставится и на `html`, и на `body`: какой из
них прокручивает страницу, решает сама страница, а с `body` он доходит до
области просмотра, только пока у `html` он `visible`. Ширина полосы прокрутки
компенсируется `padding-inline-end` (не `padding-right`: в RTL полоса слева),
иначе страница дёргается на открытии. Документ берётся у корня, а не из
`globalThis`: компонент живёт и в `iframe`. Свой счёт плагин отдаёт не на
закрытии, а когда корень доиграл переходы закрытия
(`scroll-lock/transitions.ts`): закрытая панель ещё гаснет или уезжает, и
вернувшаяся полоса сузила бы область просмотра — окно по центру съехало бы на
полширины полосы, панель у края — на всю. Переходы плагин берёт кадром позже,
когда адаптер уже перерисовал разметку, — только свои (без `subtree`) и
только `CSSTransition`: у бесконечной анимации конца нет. Отменённый переход
— тоже конец; переходов нет или нет Web Animations (jsdom) — замок уходит в
том же кадре. Открыли снова, пока плагин ждёт, — ожидание отменяется, а счёт
не трогается; снятый корень и `destroy()` отпускают сразу. Таймера с
длительностью темы нет: чисел темы плагин не знает. Подложку он не ждёт: у
модального слоя корень и есть панель, подложка по контракту темы идёт с ней
одной длительностью (сторожат «окно и подложка идут вместе» и «подложка идёт
вместе с панелью» в браузерных спеках Dialog и Drawer), а искать её в DOM
значило бы завести второй путь к разметке слоя. Сторожат
`plugins/__tests__/scroll-lock.plugin.spec.ts` и «прокрутка страницы» в
`playground/vue/browser/dialog.spec.ts` и `drawer.spec.ts`: пока панель
исчезает, прокрутка заперта, а панель не сдвинута.

**Фон модального слоя прячется по слоям, а не снимком.** Пометка —
`aria-hidden`, а не `inert`: `inert` отнимает у фона и фокус, а модель фокуса
при закрытии сразу возвращает его в фон — под `inert` `focus()` молча не
сработал бы, и работа зависела бы от того, какой из двух плагинов раньше
узнал о закрытии. Доступными остаются панель и слои выше неё — по тому же
правилу, по которому `TDismissPlugin` считает нажатие в них нажатием внутри
(`plugins/src/utils/isAboveLayer.ts`): список Select, открытый из окна, —
тоже панель в `body`, соседка окна. Снимок «все дети `body`, кроме панели»
запер бы его: скрытый `TFrame` лежит в `body` уже смонтированным, а
`data-layer` пишет только показ, и скрытие его не снимает — у скрытой панели
номера нет или он остался от прошлого показа. Поэтому, пока окно открыто,
`MutationObserver` на `body` следит за составом поддерева и за `data-layer`:
набор пересчитывается заново, а в DOM пишется только разница. Пометка у узла
одна на все слои, со счётчиком
(`plugins/src/custom/overlay/hide-outside/marks.ts`), как замок прокрутки:
окно поверх окна прячет и нижнее, закрытие одного не снимает пометок
другого, а последнее снятие возвращает то `aria-hidden`, что стояло до
первой пометки. Исключений для живых областей нет — фон прячется целиком,
как у нативного `<dialog>`. `aria-modal` плагин не пишет: это значение, и
пишет его в свой `aria` ядро компонента (см. «Прочее» в «Доступность
(a11y)»). Сторожат `plugins/__tests__/hide-outside.plugin.spec.ts` и
`ui/vue/__tests__/hide-outside.spec.ts` (Select в модальном `Frame`).
...
- **Поверхность** — `TSurface.of(descriptor, profile)`: публичный API компонента
...
- **Обмен** — `adapter.connect(profile)` на монтирование отдаёт `TExchange`:
  состояние для фреймворка (`state.subscribe` / `state.getSnapshot`), входы
  (`inputs.full` / `inputs.delta` / `offer` одного пропа), проброс событий и
  моделей (`events.listen`) и спред несъеденных пропсов (`forward`).
  Адаптеру остаётся сказать, куда положить значение и как отдать событие.
  Своих циклов по свойствам у адаптера нет. Внутри — линии (`TLine`: проп ×
  владелец × имя во фреймворке) и три порта над ними; вся память — ячейки
  (`TCell`). Новое правило о значении кладите в линию, правило «что считать
  изменением» — во вход (`TInput`); `pluginProps` — не ветка в записи, а
  обычное свойство участника-связки (`TExternalPlugins`). См. «Структура
  `packages/setup`».
  ...
  **Модель** — событие двусторонней привязки (`update:<prop>` у Vue) —
  ...
  `exportEvents`, эмитит связка в `events.listen` после события ядра и только
  на изменение.
  ...
  **`inputs.full` получает полный набор пропсов**: отсутствующий в нём проп значит
  ...
  `inputs.delta`: он трогает лишь свойства, чей ключ есть в объекте. Дельта в
  `inputs.full` сбросила бы к умолчанию всё, чего в ней нет. Из полного набора
  `inputs.full` пишет только проп, сменившийся с прошлого набора: вход сверяет его
  с последним значением фреймворка по содержимому, тем же правилом `sameValue`,
  что и состояние. React, Solid и Svelte отдают набор на каждом проходе,
  и повтор откатил бы к разметке то, что с тех пор поменяли ядро или код через
  инстанс, — список, открытый кликом при `open={false}`, закрылся бы от смены
  плейсхолдера. Литерал массива (`value={['a', 'b']}`) на каждом проходе —
  новый объект, и сверка по ссылке так же вернула бы к разметке выбор, который
  пользователь успел поменять. Сверка поверхностная: массив новых объектов
  (`items` литералом) — смена. Так в ядро во всех адаптерах пишется только то, что
...
```ts
// 1. valueOf() отдаёт снимок — его берёт TPropSpec.read(): у объекта со своим valueOf() — результат вызова
...
Сеттер — единственный, кто решает «то же самое»: обмен пишет всё, что до него
дошло (см. «Структура `packages/setup`»). Поэтому:

- **сверка — со своим записанным значением, а не с итогом.** У свойства с
  резольвером (`disabled` элемента, `size` и `variant` элемента, толщина
  Spinner, `value` Slider) своё лежит в `rawValue` единицы состояния, и своё,
  равное итогу, обязано записаться;
- **событие — о смене итога.** Своё, не сменившее итог, пишется молча, а итог,
  сменившийся от внешних данных резольвера, шлёт событие свойства
  (`TStateUnit.notify`): толщина Spinner при `'auto'` сообщает
  `change:borderWidth` и от смены размера. Второго свойства под итог
  (`resolved*`) не заводят: геттер свойства и есть итог;
- **составное — по содержимому.** Эхо модели (`update:value` вернулся тем же
  списком, `update:items` — теми же экземплярами) — не смена: `value` контрола
  сверяется поэлементно (`sameValue` в `common/state-unit/same-value.ts`),
  `items` — по экземплярам и порядку, `select` выбранного и `activate`
  активного молчат. Операции коллекции (`batch.update`) не сверяют: они
  выполняют то, что дали.

Сторожит `packages/setup/__tests__/setter-own-value.spec.ts`: по всем
дескрипторам экспорта каждому записываемому пропу — своему, плагинному и
фасадов коллекций — пишется его же значение, составное новым объектом, а
булево — после переключения, и триггеры молчат.
...
(`createAdapterContext`, шаг 5; правило — `TLine.seed`). Пропсы каждый адаптер и
...
плагинам (их создают после инстанса) сборка пишет сеттером через линию, и
...
До адаптера его доносит описание пропа — поле `TPropSpec.default` (значим ключ: `hasDefault`).
...
Поверхность (`TSurface.of(...)`) кладёт `default` в конфиг пропа
(`exportProps`) — Vue (`useProps`) раскладывает его как есть. Снятый проп во
всех шести адаптерах сбрасывает к умолчанию линия обмена
(`TLine.reset`, см. «Две поверхности управления»): она держит описание,
и копии умолчания на свойстве поверхности нет. Статическому слою остальных
адаптеров умолчание не нужно: отсутствующий проп у них `undefined`, и что с
ним делать, решает обмен.
...
**На шину, которая живёт дольше набора, плагин подписывается методом базы.**
Шина владельца переживает набор, когда приложение передаёт свой `ctrl`:
каждое монтирование ставит ему новый набор. Шины расширений движка переживают
его, когда движок пришёл снаружи (`engine`). На них — и на любой инстанс ядра,
которого плагин не создавал (панель «…» у `TTagsOverflowPlugin`), — плагин
подписывается через `TBasePlugin._listenTo`: метод запоминает отписку, а
`destroy()` базы снимает все. Своя пара `on`/`off` для этого не нужна: забытый
`off` не виден — обработчик уничтоженного плагина ничего не делает, и шина
копит его молча. На шины плагинов — своего набора и наборов элементов —
подписываются прямым `on`: они умирают вместе с наборами. Привязка к
открытости оверлея (`bindOverlayOpen`) владеет своим обработчиком и снимает
его сама (`unbind()`). Владельца, которого плагин знает по контракту, а не по
классу, описывает `IListenable`: так `TListHeightPlugin` стоит и на ListBox, и
на Select. Сторож — `setup/__tests__/plugin-unsubscribe.spec.ts`: по всем
дескрипторам экспорта подписки на шины владельца и расширений движка после
уничтожения набора сняты.
...
читает его сборка компонента (`createAdapterContext`) из пропсов — ни
...
- **Путь один, когда бы плагин ни встал.** Набор сообщает `use` и `remove`
  (`TPluginBundle.events`), набор монтирования (`TExternalPlugins`) подключает плагин
...
**Имя слота не совпадает с именем входа.** В React, Solid и Svelte слот — это
проп компонента, и слот, названный как незащищённый проп, слился бы с ним: тип
сузился бы до значения пропа (у `text` — до `string`, разметку туда не
передать), а строка ушла бы и в ядро, и в слот — второй путь к тому же тексту.
Проп — свойство класса ядра, а имя слота одно на шесть адаптеров, поэтому
меняется слот. Имя берётся по порядку: `default`; занят — `content`; занят и
он — `caption` или `header`. У `Label` `default` занят контролом, поэтому текст
подписи — слот `content`, а без него — проп `text`. Сторож —
`packages/setup/__tests__/slot-prop-names.spec.ts`: у дескрипторов экспорта ни
один слот не называется как незащищённый проп, свой или плагина.
...
`packages/ui/vue/__tests__/slots.spec.ts` разбирает компилятором Vue шаблон
каждого компонента `src/components/**`: каждый `<slot>` объявлен в дескрипторе
компонента, и ключи scope — привязки `<slot>` — те же. Дескриптор шаблону
назначает таблица в тесте, и `.vue` без строки в ней роняет тест. Раньше
сверялись семь компонентов, и слоты Input, CheckBox, Switch, Tabs.Item,
Accordion.Item и DragAndDrop жили только в разметке Vue: при переносе на React,
Solid или Svelte они не попали бы в тип пропсов. Обратная сверка — «ровно слоты
дескриптора» — только у части компонентов: `default` объявлен у ComponentView и
достаётся всем визуальным, в том числе тем, у кого в разметке его нет (Icon,
Input, CheckBox, Switch, Slider), а снять унаследованный слот объявлением
наследника нельзя.
...
- **Tags** — роли по режиму выбора. В `none` набор — `list`/`listitem`: у
  ...
  **Тегам, которым не хватило ширины ряда, отвечает одно свойство —
  `overflow`** (`wrap` по умолчанию, `scroll`, `arrows`, `popover`): пара
  свойств («переносить» + «чем листать») давала бы состояния, которых не
  бывает. В теме оно состоянием, `data-overflow`. В `arrows` ряд листают
  кнопки: он завёрнут в ленту `Scroller`, и рядом становится её вьюпорт.
  **В однострочных режимах (`scroll`, `arrows`, `popover`) тег натуральной
  ширины**: не поместившееся прокручивается, листается или уезжает в панель,
  а не сжимается. В поле Select тоже — многоточием тег там сжимается только в
  `wrap`, где одному тегу переносить некуда. **Тег под фокусом у края ряда
  `scroll` виден целиком**: частично видимый элемент браузер при фокусе не
  докручивает вовсе, а целиком скрытый ставит по центру, и срезанным
  оставался то крестик, то тег. Фокус с клавиатуры (`:focus-visible`) доводит
  `TTagsScrollPlugin` — ближайшим сдвигом (снапа у ряда нет) и весь тег, а не
  элемент под фокусом: кольцо в режиме выбора рисует пилюля. Окно ряда —
  паддинг-бокс без `scroll-padding-inline` темы, счёт общий с лентой
  (`nearestShift`, `scrollWindowOf` в `plugins/src/utils`). **Ряд `scroll` в
  поле Select — без полосы прокрутки**: поле там постоянной высоты, и
  классическая полоса торчала под его нижней рамкой. Листают ряд пальцем,
  трекпадом и колесом с Shift, а где ещё есть теги, подсказывает маска у
  края, как у ленты. Ширину подсказки ведёт сама прокрутка ряда
  (`animation-timeline: scroll()`), и состояния «есть что листать» у ряда нет;
  отступ прокрутки той же ширины делает окном ряда его чистую часть, и
  подсказка не накрывает крестик под фокусом. Видимая навигация мышью —
  `arrows`. В `popover` ряд стоит в одну
  строку, а хвост уезжает в панель Popover за кнопкой «…»: кто в ряду, а кто
  в панели, держит расширение коллекции `overflow` (`fitted` / `overflowed`),
  сколько помещается — считает `TTagsOverflowPlugin` и сообщает числом
  ...
  последний тег. **Место кнопки «…» в ряду — тоже знание коллекции**
  (`moreOrder`): ряд — флексбокс, и свой номер в коллекции тег несёт стилем
  (`order`), чтобы перестановка не переписывала разметку. Кнопка элементом
  коллекции не является, и без такого же номера она встаёт нулевой — то есть
  сразу за первым тегом, хотя в разметке стоит после всех. Её номер — номер
  первого тега, который не поместился. Стоит она **в конце строки**, а не
  вплотную за последним тегом: остаток забирает автоотступ темы, и он
  логический (`margin-inline-start`) — в RTL концом строки становится левый
  край, и кнопка уезжает туда сама. В поле Select ряд — доля строки поля, и
  кнопка стоит у её конца, вплотную к вводу: свободное место доли остаётся
  между тегами и кнопкой, а не между кнопкой и вводом. **Размер панели
  хвоста держит панель Popover** — её потолки ширины и высоты и её прокрутка
  ...
  **Тег в панели остаётся натуральной ширины**: по ней замер и помнит, сколько
  ...
  а не многоточием, как тег поля Select в `wrap`. Сторожат
  `core/__tests__/tags-overflow.spec.ts`,
  `plugins/__tests__/tags-overflow.plugin.spec.ts`,
  `plugins/__tests__/tags-scroll.plugin.spec.ts`,
...
- **Tooltip** — Tooltip pattern: `role="tooltip"` и `id` на панели, фокус
  остаётся на триггере, кнопки закрытия нет. Отдельный компонент, а не режим
  Popover: роль и модель фокуса у них разные, общий у них только слой оверлея
  (см. «Граница переиспользования»). Триггер — слот `trigger`, как у Popover;
  ссылку на панель он получает в scope (`triggerAria`) — всегда, а не только у
  открытой: панель не размонтируется (открытость — её `visible`), и описание
  есть уже в первой отрисовке и не запаздывает за фокусом. Поэтому
  `lazyMount` у подсказки нет. **Подсказка — описание или имя триггера**
  (`type`, как у GitHub Primer). `description` (по умолчанию) — паттерн APG:
  ссылка `aria-describedby`, имя у триггера своё — текст или `aria_label`.
  `label` — подсказка и есть имя: ссылка `aria-labelledby`, одна, без
  `aria-describedby`. Это режим кнопки-иконки без текста: `aria_label` ей не
  нужен, а с ним и подсказкой-описанием скринридер прочёл бы один текст
  дважды. Панель от режима не зависит — та же `role="tooltip"`, так же всегда
  в документе: скрытый узел, на который прямо ссылается `aria-labelledby`, в
  имя входит (accname 1.2, шаг 2A). `aria_label` триггера компонент не
  снимает и не проверяет: у содержимого слота нет экземпляра, а совмещать имя
  и подсказку — решение потребителя. Когда показывать, решает
  `TTooltipTriggerPlugin`, и держат подсказку две причины: курсор на корне
  или панели (не касание) — показ через `openDelay` (400 мс), скрытие через
  `closeDelay` (150 мс), панель под курсором держит (WCAG 1.4.13); фокус с
  клавиатуры (`:focus-visible`) — показ и скрытие сразу. Нажатие на корень и
  Escape прячут подсказку и гасят обе причины вместе с отложенным показом:
  снова её покажет только новый заход курсора или новый фокус. Escape
  слушается на документе в фазе перехвата, пока подсказка открыта, и гасится
  `preventDefault`, а не `stopPropagation`: поповер под подсказкой клавишу
  пропустит. Слушает плагин корень, а не триггер: у выключенной кнопки
  `pointer-events: none`, и курсор попадает в корень. Нажатие мимо —
  `TDismissPlugin` без `focusOutside`. Подсказка на экране одна на документ:
  открывшаяся закрывает прежнюю, а пока другая открыта или закрылась меньше
  300 мс назад, следующая показывается без задержки — состояние документа, а
  не плагина (`tooltip/trigger/document-state.ts`, как замок прокрутки).
  Сторожат `core/__tests__/tooltip.spec.ts`,
  `plugins/__tests__/tooltip-trigger.plugin.spec.ts`,
  `ui/vue/__tests__/tooltip.spec.ts` и `playground/vue/browser/tooltip.spec.ts`.
- **Dialog** — Dialog (Modal) pattern: `role="dialog"` и `aria-modal="true"`
  на панели, имя — видимый заголовок (`aria-labelledby` на слот `title`,
  который рисуется всегда; `id` он берёт из выхода `titleAria`). `TAriaPlugin`
  окну не ставится: он пишет те же ключи и снял бы связку. Немодальный диалог
  у нас — Popover, поэтому флага `modal` нет. Триггера нет: окно открывает
  код или любая кнопка, открытость — `visible` (`v-model:visible`), а не
  `open`: окно само и есть слой (`TDialog` — модальный слой `TModalLayer`,
  наследник `TLayer`), и `open` рядом с `visible` был бы вторым путём к
  одному факту. Корень — сама панель, а
  подложка — её сосед в телепорте, раньше в DOM, с тем же `z-index`
  (`layout_backdropStyles`) и номером слоя (`backdropDataset`); пометки
  владельцем у подложки нет — для своего окна нажатие по ней мимо. Позиционер
  на весь экран вокруг панели развёл бы слой и панель по разным узлам, и клик
  в список Select внутри закрыл бы окно. Подложка — обязательная разметка:
  фон спрятан от скринридера (`THideOutsidePlugin`), но не от указателя, и к
  ...
  **Место, размер и разворот — значения, раскладка — тема**, без замеров и
  ...
  стоит всегда; `width`/`height` (число — px, строка — CSS-значение, умолчание
  `undefined`, а не `'auto'`) — переменными `--dialog-width`/`--dialog-height`;
  `offset` — отступ от краёв экрана (число — px, строка — CSS-значение,
  `undefined` — отступ темы, `0` — вплотную) — переменными
  `--dialog-offset-{top,bottom,start,end}` по логическим сторонам, только
  заданным; `maximized` — `data-maximized`, под которым тема не читает ни
  место, ни размер, ни отступ. Отступ — поля области экрана, в которую тема
  вписывает окно: у центра окно по центру области, у стороны — у её края, и
  всегда область — потолок размера. Тема ставит отступы в `inset` панели, а
  не в поля: процент поля считается от ширины экрана и по вертикали. Стороны
  по отдельности правит подписчик `layout:offset:before` —
  `TDialogOffsetEvent`, `TActionEvent` с местом (`placement`) и четырьмя
  изменяемыми сторонами из `offset`, как `TInsertEvent` у коллекций;
  `preventDefault()` отдаёт все стороны теме. Событие приходит на каждый
  пересчёт стилей окна, и в `created()` плагина тоже — после `bundle:create`
  и `layout:create`, до первого кадра: поправку получает и окно, созданное
  видимым. Кнопка разворота (`maximizable`, по умолчанию выключена) —
  переключатель: имя одно (`maximizeLabel`), состояние — `aria-pressed`, пишет
  она тот же `maximized`, что и разработчик. Крестик (`closable`, имя
  `closeLabel`) — запрос `requestClose('button')`. Нажатие по подложке и
  Escape — тоже запросы (см. «Закрытие пользователем — запрос» в «Слой
  оверлея»): `dismissible: false` оставляет окну только кнопку, отмена
  `close:before` — любое своё правило. Кнопки стоят в DOM последними, в ряд
  заголовка их переносит тема: фокус при открытии попадает на первый контрол
  содержимого, крестик — последняя остановка Tab. Предупреждение (`alert`) —
  режим того же окна, а не компонент: `role="alertdialog"` и
  `aria-describedby` на тело, клавиатура и фокус те же. Сторожат
  `core/__tests__/dialog.spec.ts`,
  `plugins/__tests__/modal-focus.plugin.spec.ts`,
  `plugins/__tests__/dialog-layout.plugin.spec.ts`,
  `ui/vue/__tests__/dialog.spec.ts` и `playground/vue/browser/dialog.spec.ts`.
- **Drawer** — выезжающая панель у края экрана. Своего паттерна APG у неё нет:
  это модальный Dialog, и модальность у неё общая с окном (`TModalLayer`, см.
  «Слой оверлея») — роль, `aria-modal`, имя от заголовка, крестик, Escape,
  подложка, `dismissible`, `close:before`, фокус, немой фон. Модальна панель
  всегда, в том числе внутри контейнера. Анатомия та же, что у окна: корень —
  сама панель, подложка — её сосед. **Край — значение, раскладка и анимация —
  тема**: `placement` (`end` по умолчанию, `start`, `top`, `bottom` —
  логические стороны, центра нет) — модификатором `--placement-<v>`, размер по
  оси края — `width` у боковой, `height` у верхней и нижней, переменными
  `--drawer-width`/`--drawer-height`. Въезд и выезд — переход темы:
  `data-open` у панели и подложки говорит, куда ехать, `@starting-style` даёт
  въезд, `transition-behavior: allow-discrete` откладывает `display: none` до
  конца выезда. Выезд одинаков при любых настройках системы, и при
  `prefers-reduced-motion` тоже — решение владельца: это обычный сдвиг панели
  от края, так она открывается и закрывается. Ход ручек Slider и листание
  Scroller просьбу системы выполняют. «Присутствия» в ядре и `<Transition>` в
  адаптере нет. **Жест** (`swipe`: `none` по умолчанию,
  `handle` — за полосу у края, `panel` — за любое место, кроме
  контролов (`closestControl`) и областей, которые прокручиваются вдоль оси)
  ведёт `TDrawerSwipePlugin`: нажатие становится жестом, когда указатель ушёл
  вдоль оси дальше порога (поперёк — не жест), к краю панель идёт за
  указателем, от края — с сопротивлением; отпустили дальше четверти размера
  или быстро к краю — запрос `requestClose('swipe')`, иначе и при отменённом
  `close:before` панель возвращается. `start`/`end` в RTL зеркалятся по
  вычисленному направлению. Пока жест включён, плагин ставит панели
  `touch-action` по оси — поведение не зависит от CSS темы. Полоса — разметка,
  не часть: потребитель её не адресует, скринридеру она не нужна
  (`aria-hidden`), рисуется по выходу `handleRendered`, зона захвата — не
  меньше 44px. Вариант без перетаскивания (WCAG 2.5.7) — крестик, подложка и
  Escape; жест закрывает и при `dismissible: false` — его включают сами.
  **Внутри контейнера** (`contained`) панель не телепортируется, а встаёт в
  ближайшем позиционированном предке (`data-contained`) — например, в
  модальном окне, где она слой выше окна. Прокрутку документа она тогда не
  запирает: `TScrollLockPlugin` привязан к `locksScroll`. `<dialog>` и
  `showModal()` не берутся: верхний слой лежит над всем документом, и
  `contained` пропал бы. Сторожат `core/__tests__/drawer.spec.ts`,
  `plugins/__tests__/drawer-swipe.plugin.spec.ts`,
  `plugins/__tests__/drawer-layout.plugin.spec.ts`,
  `ui/vue/__tests__/drawer.spec.ts` и `playground/vue/browser/drawer.spec.ts`.
...
доносит сборка (`createAdapterContext`, шаг 5) — ядро получает пропсы через
...
- Angular AOT требует статические массивы `inputs` и `outputs` в декораторе,
...
- **Входы и выходы с их типами объявляет тот же генератор**: рядом с массивами
  имён он пишет абстрактную директиву `T<Имя>Surface` — поверхность
  компонента в Angular. Её декоратор объявляет входы и выходы (`inputs`,
  `outputs`), а поля `declare` дают им тип: входу — `TInputValue`, тип пропа
  по фабрике дескриптора и имени входа (свой проп, проп плагина или служебный
  `embedded` и `pluginProps`, как их сводит `DescriptorComponentProps`),
  выходу — `TOutputEmitter` по полному имени события. Компонент наследует её,
  а не `TComponentBase`, и своих `inputs` и `outputs` в `@Component` не
  объявляет. Строгая проверка шаблона потребителя (`strictTemplates` —
  умолчание новых приложений) читает выход как поле класса: без поля привязка
  `(actionPress)` падает с TS7053, а `$event` получает тип поля — первый
  аргумент события ядра, его и шлёт `syncEvents`. Значение привязки входа она
  сверяет с полем входа — число в `[text]`, размер вне шкалы, вид, которого
  тема не объявила, стали ошибками шаблона (булев вход сверяется иначе, см.
  ниже), — но вход без поля пропускает молча, а поле ищет только среди
  собственных членов класса, чей декоратор объявил вход. Поэтому вход
  объявляет тот же класс, что его поле: объяви компонент свои `inputs` ещё
  раз, сверка пропала бы незаметно, хотя поле в базе есть. Поле входа — без
  `readonly`: с ним привязка при `strictInputAccessModifiers` была бы
  ошибкой. В рантайме полей `declare` нет: значение входа пишет Angular,
  эмиттеры ставит `TComponentBase` по массиву имён. Тип фабрики класс берёт
  из манифеста, поэтому `descriptor` там — реэкспорт из `@soldy-ui/setup`, а
  не `const`: выведенный тип `const` декларации пакета выписать не могут
  (TS2883, TS7056). Сторожа — `outputs.spec.ts` и `inputs.spec.ts` в
  `packages/ui/angular/__tests__`: хосты с верными привязками (их шаблоны
  проверяет «Типы — Angular») и `expectTypeOf` на тип поля — с `any` шаблон
  прошёл бы молча. Неверное значение в каждом входе каждого компонента
  экспорта `inputs.spec.ts` компилирует компилятором Angular в памяти и ждёт
  ошибку на каждой привязке: отрицательный случай шаблона под
  `@ts-expect-error` не записать, а вход, объявленный ещё раз в `@Component`,
  видно только по неверному значению, которое прошло.
- **Булев вход принимает атрибут без значения**, как в остальных адаптерах:
  `<soldy-button disabled>` выключает кнопку. Атрибут без значения Angular
  отдаёт входу пустой строкой, поэтому вход, чей проп объявлен ровно `Boolean`
  (`useBooleanInputs`), генератор объявляет в декораторе `T<Имя>Surface`
  объектом с `transform: booleanInput`
  (`packages/ui/angular/src/adapter/runtime/boolean-input.ts`): `''`
  становится `true`, остальное проходит как есть. Transform Angular
  применяет к любой записи входа — атрибуту, привязке, `setInput` — до
  `ngOnChanges` и до записи в поле, поэтому `TComponentBase` о нём не знает, а
  поле входа (`TInputValue`) — тип значения после transform. Тип записи
  проверка шаблона берёт у параметра transform, в декларации пакета это
  `ngAcceptInputType_<вход>`. Параметр — ровно `boolean | '' | undefined`:
  `disabled` без значения проходит, а `disabled="yes"` и `[disabled]="'yes'"`
  остаются ошибками. `booleanAttribute` из `@angular/core` не годится: его
  параметр — `unknown`, и сверка пропала бы, а `undefined` он превращает в
  `false`, и снятый `visible` не вернулся бы к умолчанию `true`. Проп, у
  которого `Boolean` — один из типов массива (`value` полей), transform не
  получает: пустая строка у него — значение. Сторож — `inputs.spec.ts`: хост
  с `disabled` без значения выключает кнопку, снятый булев вход возвращается
  к умолчанию, а в памяти булевы входы каждого компонента экспорта атрибутом
  без значения компилируются без ошибок, со значением `"yes"` — с ошибкой на
  каждом.
...
- Спеки (`packages/ui/angular/__tests__/*.spec.ts`) гоняет официальный билдер
  ...
  in TypeScript compilation». Входы задаются `componentRef.setInput()`: поля
  входов `T<Имя>Surface` только объявляет, и запись в поле мимо Angular не
  даёт `ngOnChanges`. Цель `build` рядом с `test` — только носитель опций
  сборки, приложения у пакета нет; `buildTarget` у `test` указан явно, иначе
  билдер ищет конфигурацию `development` и падает на её отсутствии. Хосты
  спеков — `OnPush`, как любой компонент Angular 22 без явной стратегии:
  значение, которое хост меняет по ходу теста, — сигнал, иначе
  `fixture.detectChanges()` его шаблон не перепроверит.
...
- Библиотеку собирает `ng-packagr` скриптом `build` (см. «Сборка пакетов»), а
  не цель `angular.json`: цели `build` и `test` там — только прогон спеков.
  Цель `build` под сборку библиотеки не переиспользуется и не переименовывается:
  на неё явно указывает `buildTarget` у `test`, а её пустой `polyfills` держит
  прогон без Zone.js. Конфигов TS у пакета три: `tsconfig.json` — проверка
  типов (`ngc`, соседи исходниками через `paths`, `noEmit`, `dist` исключён),
  `tsconfig.spec.json` — прогон спеков, `tsconfig.build.json` — сборка (пустые
  `paths`, частичная форма).
...
- У `packages/setup` общего окружения в vitest-конфиге нет, и по умолчанию оно
  `node`. Тестам с DOM нужна первая строка `// @vitest-environment jsdom`. Сам
  конфиг держит только алиасы на исходники соседей и самого пакета.
...
имена ядра (`change:text`, `action:press`): Vue их не меняет, стенд
