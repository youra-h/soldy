...
- **Связка** — `bindComponent(adapter, profile)` на монтирование: стартовое
  ...
  **`writeAll` получает полный набор пропсов**: отсутствующий в нём проп значит
  ...
  разметку. React пересобирает контекст, когда заново устанавливает эффекты
  (StrictMode, `<Activity>`), и связка нового контекста продолжает память
  прошлой (`bindComponent` с прошлой связкой) — иначе её первый набор снова
  записал бы разметку во внешний `ctrl`. Сторож —
  `setup/__tests__/binding.spec.ts`, «связка · повторённый проп» и «связка ·
  продолжение прошлой».
