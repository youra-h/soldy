...
`TEvented.relay()` сняты сменой контракта, а не заменой на `TEventSink`.
...
уже честные HTML. Разметка биндит его рядом с `aria`: `v-bind="{ ...attrs,
...aria }"`.
...
либо `aria-disabled` в `aria` — никогда оба сразу. Раньше это условие
(`tag === 'button' ? disabled : undefined`) писал каждый шаблон Button сам, и
поменять список тегов значило бы поменять шесть шаблонов.
...
Пуст у визуального слоя самого по себе — как и `aria`. Пишет в него пока
только `TControl` (`disabled`); появится второй нативный атрибут с тем же
правилом «есть у тега / нет у тега» — он идёт сюда же, а не в третий набор.
