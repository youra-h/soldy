...
| `define/`                                  | Описание типа: `TName`, `TPropSpec`, дескриптор, определение плагина, наследование, умолчания, `defineType`, вывод типов.                        |
...
пересчитываются от итогового `ctor`, описания заморожены; о двух пропах или событиях с
одним полным именем дескриптор сообщает в консоль (`console.error`) и работает
дальше — дубль на совести автора, а плагины разводит неймспейс. Определение плагина
...
которому она понадобится. До тех пор входы `size`/`variant` у `*.Item`
остаются в поверхности и ничего не делают — снимает их отдельная задача.
